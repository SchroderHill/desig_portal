...
export function initialiseExport({button,map,draw,getMaps}) {
  ...
  dialog.innerHTML=`<form method="dialog"><button class="export-close" aria-label="Close export">×</button></form><h2>Export design</h2>
    ...
    <button type="submit" class="export-send">Send design</button>
    <button type="button" id="export-local">Download locally</button></form>
    <button type="button" id="export-cancel" hidden>Cancel</button>
    <p id="export-progress" role="status" aria-live="polite"></p>`;
  const style=document.createElement('style');style.textContent=`.design-export{box-sizing:border-box;width:min(480px,94vw);max-height:92vh;overflow:auto;border:0;border-radius:10px;padding:24px;color:#26332a;font:14px/1.4 system-ui;box-shadow:0 12px 60px #0005}.design-export::backdrop{background:#0007}.design-export h2{margin:0 0 12px;font-size:21px}.design-export h3{font-size:16px;margin:0 0 12px}.design-export label{display:block;margin:10px 0}.design-export input:not([type=checkbox]):not([type=radio]),.design-export textarea,.design-export select{box-sizing:border-box;width:100%;padding:9px;margin-top:4px;border:1px solid #bdc8bf;border-radius:4px;font:inherit}.design-export textarea{resize:vertical}.design-export button{padding:10px 14px;border:0;border-radius:5px;background:#294632;color:white;cursor:pointer}.design-export button:disabled{opacity:.5;cursor:wait}.design-export .export-close{float:right;background:transparent;color:#526257;padding:0 4px;font-size:24px}.design-export p,.design-export span{font-size:12px;color:#526257}.design-export fieldset{border:0;padding:0;margin:14px 0}.design-export legend{font-size:12px;color:#526257}.design-export .export-check{display:flex;align-items:center;gap:8px}.design-export #portal-submit-form{border-top:1px solid #dde3de;margin-top:18px;padding-top:18px}.design-export .export-send{width:100%;background:#243469}.design-export #export-local{display:block;width:100%;margin-top:10px;background:#eef2ef;color:#294632}.design-export [hidden]{display:none!important}`;
