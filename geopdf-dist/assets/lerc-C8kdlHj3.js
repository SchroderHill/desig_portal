...
import { a as wA, L as gA } from "./dem-export.worker-RFb31aY7.js";
function hA(v) {
...
function SA() {
  return iA || (iA = 1, (function(v) {
    ...
    (function() {
      var q = (function() {
        var K = {};
        K.defaultNoDataValue = -34027999387901484e22, K.decode = function(I, D) {
          D = D || {};
          var C = D.encodedMaskData || D.encodedMaskData === null, g = E(I, D.inputOffset || 0, C), r = D.noDataValue !== null ? D.noDataValue : K.defaultNoDataValue, B = Y(
            g,
          ...
          return B.resultMask && (Q.maskData = B.resultMask), D.returnEncodedMask && g.mask && (Q.encodedMaskData = g.mask.bitset ? g.mask.bitset : null), D.returnFileInfo && (Q.fileInfo = l(g), D.computeUsedBitDepths && (Q.fileInfo.bitDepths = P(g))), Q;
        };
          ...
          for (var L = new Float32Array(s * e), N, k, y = 0; y <= a; y++) {
            ...
            if (c !== 0)
              for (var G = 0; G <= Q; G++) {
                ...
                if (w !== 0) {
                  var U = y * I.width * e + G * s, t = I.width - w, n = I.pixels.blocks[B], f, R, M;
                  n.encoding < 2 ? (n.encoding === 0 ? f = n.rawData : (A(n.stuffedData, n.bitsPerPixel, n.numValidPixels, n.offset, h, L, I.pixels.maxValue), f = L), R = 0) : n.encoding === 2 ? M = 0 : M = n.offset;
                  var J;
                  if (C)
                    for (k = 0; k < c; k++) {
                      for (U & 7 && (J = C[U >> 3], J <<= U & 7), N = 0; N < w; N++)
                        U & 7 || (J = C[U >> 3]), J & 128 ? (F && (F[U] = 1), i = n.encoding < 2 ? f[R++] : M, o = o > i ? i : o, S[U++] = i) : (F && (F[U] = 0), S[U++] = g), J <<= 1;
                      U += t;
                    }
            ...
            for (var k = 0; k < S; k++) {
              ...
              if (D += y, G.encoding !== 3) {
                ...
                if (G.encoding === 0) {
                  ...
                  var R = Math.ceil(G.numValidPixels * G.bitsPerPixel / 8), M = Math.ceil(R / 4);
                  U = new ArrayBuffer(M * 4), t = new Uint8Array(U), t.set(new Uint8Array(I, D, R)), G.stuffedData = new Uint32Array(U), D += R;
                }
              }
            }
        ...
        var K = {
            ...
            for (s = r; s < B; s++)
              if (e = s - (s < g ? 0 : g), o = a[e].first, o > 0)
                if (n = [o, e], o <= w)
                  ...
                  for (f = a[e].second, H = U, M = o - 1; M >= 0; M--)
                    J = f >>> M & 1, J ? (H.right || (H.right = new l()), H = H.right) : (H.left || (H.left = new l()), H = H.left), M === 0 && !H.val && (H.val = n[1]);
            return {
              decodeLut: t,
              numBitsLUTQick: w,
              numBitsLUT: G,
          ...
          readHuffman: function(E, A, I, D) {
            var C = A.headerInfo, g = C.numDims, r = A.headerInfo.height, B = A.headerInfo.width, Q = B * r, a = this.readHuffmanTree(E, A), s = a.decodeLut, e = a.tree, h = a.stuffedData, o = a.srcPtr, i = a.bitPos, S = a.numBitsLUTQick, F = a.numBitsLUT, L = A.headerInfo.imageType === 0 ? 128 : 0, N, k, y, c = A.pixels.resultMask, G, w, U, t, n, f, R, M = 0;
            i > 0 && (o++, i = 0);
            var J = h[o], H = A.encodeMode === 1, V = new I(Q * g), m = V, d;
            if (g < 2 || H) {
              for (d = 0; d < g; d++)
                if (g > 1 && (m = new I(V.buffer, Q * d, Q), M = 0), A.headerInfo.numValidPixel === B * r)
                  for (f = 0, t = 0; t < r; t++)
                    for (n = 0; n < B; n++, f++) {
                      ...
                      i >= 32 && (i -= 32, o++, J = h[o]), y = k - L, H ? (n > 0 ? y += M : t > 0 ? y += m[f - B] : y += M, y &= 255, m[f] = y, M = y) : m[f] = y;
                    }
                else
                  for (f = 0, t = 0; t < r; t++)
                    for (n = 0; n < B; n++, f++)
                      if (c[f]) {
                        ...
                        i >= 32 && (i -= 32, o++, J = h[o]), y = k - L, H ? (n > 0 && c[f - 1] ? y += M : t > 0 && c[f - B] ? y += m[f - B] : y += M, y &= 255, m[f] = y, M = y) : m[f] = y;
                      }
            } else
              for (f = 0, t = 0; t < r; t++)
                for (n = 0; n < B; n++)
                  if (f = t * B + n, !c || c[f])
                    for (d = 0; d < g; d++, f += Q) {
                      ...
                      i >= 32 && (i -= 32, o++, J = h[o]), y = k - L, m[f] = y;
                    }
            A.ptr = A.ptr + (o + 1) * 4 + (i > 0 ? 4 : 0), A.pixels.resultPixels = V, g > 1 && !D && (A.pixels.resultPixels = Y.swapDimensionOrder(V, Q, g, I));
          },
          decodeBits: function(E, A, I, D, C) {
            {
              ...
              if (o) {
                for (A.counter.lut++, U = a.getUint8(B), B++, y = Math.ceil((U - 1) * i / 8), c = Math.ceil(y / 4), N = new ArrayBuffer(c * 4), k = new Uint8Array(N), A.ptr += B, k.set(new Uint8Array(E, A.ptr, y)), w = new Uint32Array(N), A.ptr += y, t = 0; U - 1 >>> t; )
                  t++;
                y = Math.ceil(S * t / 8), c = Math.ceil(y / 4), N = new ArrayBuffer(c * 4), k = new Uint8Array(N), k.set(new Uint8Array(E, A.ptr, y)), L = new Uint32Array(N), A.ptr += y, r >= 3 ? G = K.unstuffLUT2(w, i, U - 1, D, F, n) : G = K.unstuffLUT(w, i, U - 1, D, F, n), r >= 3 ? K.unstuff2(L, I, t, S, G) : K.unstuff(L, I, t, S, G);
              } else
                A.counter.bitstuffer++, t = i, A.ptr += B, t > 0 && (y = Math.ceil(S * t / 8), c = Math.ceil(y / 4), N = new ArrayBuffer(c * 4), k = new Uint8Array(N), k.set(new Uint8Array(E, A.ptr, y)), L = new Uint32Array(N), A.ptr += y, r >= 3 ? D == null ? K.originalUnstuff2(L, I, t, S) : K.unstuff2(L, I, t, S, !1, D, F, n) : D == null ? K.originalUnstuff(L, I, t, S) : K.unstuff(L, I, t, S, !1, D, F, n));
            }
          },
          readTiles: function(E, A, I, D) {
            ...
            var o = 0, i = 0, S = 0, F = 0, L = 0, N = 0, k = 0, y = 0, c = 0, G = 0, w = 0, U = 0, t = 0, n = 0, f = 0, R = 0, M, J, H, V, m, d, z = new I(Q * Q), DA = r % Q || Q, sA = g % Q || Q, AA, X, $ = C.numDims, W, T = A.pixels.resultMask, p = A.pixels.resultPixels, aA = C.fileVersion, EA = aA >= 5 ? 14 : 15, Z, IA = C.zMax, b;
            for (S = 0; S < h; S++)
              for (L = S !== h - 1 ? Q : DA, F = 0; F < e; F++)
                for (N = F !== e - 1 ? Q : sA, w = S * g * Q + F * Q, U = g - N, W = 0; W < $; W++) {
                  if ($ > 1 ? (b = p, w = S * g * Q + F * Q, p = new I(A.pixels.resultPixels.buffer, B * W * s, B), IA = C.maxValues[W]) : b = null, k = E.byteLength - A.ptr, M = new DataView(E, A.ptr, Math.min(10, k)), J = {}, R = 0, y = M.getUint8(0), R++, Z = C.fileVersion >= 5 ? y & 4 : 0, c = y >> 6 & 255, G = y >> 2 & EA, G !== (F * Q >> 3 & EA) || Z && W === 0)
                    throw "integrity issue";
                  if (d = y & 3, d > 3)
                    throw A.ptr += R, "Invalid block encoding (" + d + ")";
                  if (d === 2) {
                    if (Z)
                      if (T)
                        for (o = 0; o < L; o++)
                    ...
                    if (Z)
                      throw "integrity issue";
                    if (A.counter.uncompressed++, A.ptr += R, t = L * N * s, n = E.byteLength - A.ptr, t = t < n ? t : n, H = new ArrayBuffer(t % s === 0 ? t : t + s - t % s), V = new Uint8Array(H), V.set(new Uint8Array(E, A.ptr, t)), m = new I(H), f = 0, T)
                      for (o = 0; o < L; o++) {
                        for (i = 0; i < N; i++)
                          T[w] && (p[w] = m[f++]), w++;
                        w += U;
                    ...
                    A.ptr += f * s;
                  } else if (AA = Y.getDataTypeUsed(Z && a < 6 ? 4 : a, c), X = Y.getOnePixel(J, R, AA, M), R += Y.getDataTypeSize(AA), d === 3)
                    if (A.ptr += R, A.counter.constantoffset++, T)
                      for (o = 0; o < L; o++) {
                        for (i = 0; i < N; i++)
                          T[w] && (p[w] = Z ? Math.min(IA, b[w] + X) : X), w++;
                        w += U;
                      }
                    else
                      for (o = 0; o < L; o++) {
                        for (i = 0; i < N; i++)
                          p[w] = Z ? Math.min(IA, b[w] + X) : X, w++;
                        w += U;
                      }
                  else if (A.ptr += R, Y.decodeBits(E, A, z, X, W), R = 0, Z)
                    if (T)
                      for (o = 0; o < L; o++) {
                        for (i = 0; i < N; i++)
                          T[w] && (p[w] = z[R++] + b[w]), w++;
        ...
        var K = new ArrayBuffer(4), Y = new Uint8Array(K), l = new Uint32Array(K);
        return l[0] = 1, Y[0] === 1;
      })(), u = {
        /************wrapper**********************************************/
        /**
        ...
        **/
        decode: function(K, Y) {
          ...
          Y = Y || {};
          var l = Y.inputOffset || 0, P = new Uint8Array(K, l, 10), E = String.fromCharCode.apply(null, P), A, I;
          if (E.trim() === "CntZImage")
            A = q, I = 1;
          ...
          for (var D = 0, C = K.byteLength - 10, g, r = [], B, Q, a = {
            width: 0,
            height: 0,
            pixels: [],
            ...
            l = e.fileInfo.eofOffset, Q = e.maskData, D === 0 && (g = e.encodedMaskData, a.width = e.width, a.height = e.height, a.dimCount = e.dimCount || 1, a.pixelType = e.pixelType || e.fileInfo.pixelType, a.mask = Q), I > 1 && (Q && r.push(Q), e.fileInfo.mask && e.fileInfo.mask.numBytes > 0 && s++), D++, a.pixels.push(e.pixelData), a.statistics.push({
              minValue: e.minValue,
              maxValue: e.maxValue,
              noDataValue: e.noDataValue,
              dimStats: e.dimStats
      ...
      v.exports ? v.exports = u : this.Lerc = u;
    })();
  })(BA)), BA.exports;
}
var GA = SA(), NA = /* @__PURE__ */ hA(GA);
let _, x, QA;
const CA = {
  env: {
    emscripten_notify_memory_growth: (v) => {
...
class yA {
  init() {
    return _ || (typeof fetch < "u" ? _ = fetch(`data:application/wasm;base64,${oA}`).then((q) => q.arrayBuffer()).then((q) => WebAssembly.instantiate(q, CA)).then(this._init) : _ = WebAssembly.instantiate(Buffer.from(oA, "base64"), CA).then(this._init), _);
  }
  _init(q) {
    x = q.instance, CA.env.emscripten_notify_memory_growth(0);
  }
  decode(q, j = 0) {
    if (!x) throw new Error("ZSTDDecoder: Await .init() before decoding.");
    const O = q.byteLength, u = x.exports.malloc(O);
    QA.set(q, u), j = j || Number(x.exports.ZSTD_findDecompressedSize(u, O));
    const K = x.exports.malloc(j), Y = x.exports.ZSTD_decompress(K, j, u, O), l = QA.slice(K, K + Y);
    return x.exports.free(u), x.exports.free(K), l;
  }
}
...
class kA extends rA {
   ...
   */
  decodeBlock(q) {
    const O = /** @type {LercDecoderParameters} */ this.parameters.LercParameters?.[wA.AddCompression];
    let u = q;
    switch (O) {
      case gA.None:
        break;
      case gA.Deflate:
        u = eA(new Uint8Array(u)).buffer;
        break;
      case gA.Zstandard:
        u = FA.decode(new Uint8Array(u)).buffer;
        break;
      default:
        throw new Error(`Unsupported LERC additional compression method identifier: ${O}`);
    }
    return NA.decode(u, { returnPixelInterleavedDims: this.parameters.planarConfiguration === 1 }).pixels[0].buffer;
  }
}
export {
  kA as default,
