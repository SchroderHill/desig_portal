...
var he = 1, fe = 2, ve = 3, Rs = 4, ja = 5, mt = 6378137, Os = 6356752314e-3, ut = 0.0066943799901413165, Ne = 484813681109536e-20, _ = Math.PI / 2, Ds = 0.16666666666666666, Fs = 0.04722222222222222, Ls = 0.022156084656084655, g = 1e-10, U = 0.017453292519943295, V = 57.29577951308232, P = Math.PI / 4, Fe = Math.PI * 2, I = 3.14159265359, H = {};
H.greenwich = 0;
H.lisbon = -9.131906111111;
H.paris = 2.337229166667;
H.bogota = -74.080916666667;
H.madrid = -3.687938888889;
H.rome = 12.452333333333;
H.bern = 7.439583333333;
H.jakarta = 106.807719444444;
H.ferro = -17.666666666667;
H.brussels = 4.367975;
H.stockholm = 18.058277777778;
H.athens = 23.7163375;
H.oslo = 10.722916666667;
var qs = {
  mm: { to_meter: 1e-3 },
  cm: { to_meter: 0.01 },
  ft: { to_meter: 0.3048 },
...
function oe(e, a) {
  if (e[a])
    return e[a];
  for (var t = Object.keys(e), s = a.toLowerCase().replace(bt, ""), n = -1, i, d; ++n < t.length; )
    if (i = t[n], d = i.toLowerCase().replace(bt, ""), d === s)
      return e[i];
}
function Na(e) {
...
function Ks(e) {
  return ($s(e) === "2019" ? Hs : Vs).convert(e);
}
function Xs(e) {
...
var Le = 1, Ft = 2, Lt = 3, ma = 4, qt = 5, Ha = -1, Ys = /\s/, Ws = /[A-Za-z]/, Js = /[A-Za-z84_]/, Ma = /[,\]]/, Vt = /[\d\.E\-\+]/;
function se(e) {
  if (typeof e != "string")
    throw new Error("not a string");
  this.text = e.trim(), this.level = 0, this.place = 0, this.root = null, this.stack = [], this.currentObject = null, this.state = Le;
}
se.prototype.readCharicter = function() {
...
se.prototype.afterquote = function(e) {
  if (e === '"') {
    this.word += '"', this.state = ma;
    return;
  }
  if (Ma.test(e)) {
...
se.prototype.afterItem = function(e) {
  if (e === ",") {
    this.word !== null && this.currentObject.push(this.word), this.word = null, this.state = Le;
    return;
  }
  if (e === "]") {
    this.level--, this.word !== null && (this.currentObject.push(this.word), this.word = null), this.state = Le, this.currentObject = this.stack.pop(), this.currentObject || (this.state = Ha);
    return;
  }
};
se.prototype.number = function(e) {
  if (Vt.test(e)) {
    this.word += e;
    return;
  }
...
se.prototype.quoted = function(e) {
  if (e === '"') {
    this.state = qt;
    return;
  }
...
se.prototype.keyword = function(e) {
  if (Js.test(e)) {
    this.word += e;
    return;
  }
  if (e === "[") {
    var a = [];
    a.push(this.word), this.level++, this.root === null ? this.root = a : this.currentObject.push(a), this.stack.push(this.currentObject), this.currentObject = a, this.state = Le;
    return;
  }
  if (Ma.test(e)) {
    this.afterItem(e);
...
se.prototype.neutral = function(e) {
  if (Ws.test(e)) {
    this.word = e, this.state = Ft;
    return;
  }
  if (e === '"') {
    this.word = "", this.state = ma;
...
function ue(e, a) {
  ...
  switch (t) {
    ...
    case "ENGCRS":
    case "ENGINEERINGCRS":
      e[0] = ["name", e[0]], Sa(a, t, e), a[t].type = t;
      return;
    default:
      for (s = -1; ++s < e.length; )
...
var Zs = 0.017453292519943295;
function X(e) {
  return e * Zs;
}
function Ht(e) {
  const a = (e.projName || "").toLowerCase().replace(/_/g, " ");
  e.long0 === void 0 && e.longc !== void 0 && (e.long0 = e.longc), !e.lat_ts && e.lat1 && (a === "stereographic south pole" || a === "polar stereographic (variant b)") ? (e.lat0 = X(e.lat1 > 0 ? 90 : -90), e.lat_ts = e.lat1, delete e.lat1) : !e.lat_ts && e.lat0 && (a === "polar stereographic" || a === "polar stereographic (variant a)") && (e.lat_ts = e.lat0, e.lat0 = X(e.lat0 > 0 ? 90 : -90), delete e.lat1);
}
function wt(e) {
...
function pt(e, a) {
  e.ellipsoid.radius ? (a.a = e.ellipsoid.radius, a.rf = 0) : (a.a = zt(e.ellipsoid.semi_major_axis), e.ellipsoid.inverse_flattening !== void 0 ? a.rf = e.ellipsoid.inverse_flattening : e.ellipsoid.semi_major_axis !== void 0 && e.ellipsoid.semi_minor_axis !== void 0 && (a.rf = a.a / (a.a - zt(e.ellipsoid.semi_minor_axis))));
}
function ua(e, a = {}) {
  return !e || typeof e != "object" ? e : e.type === "BoundCRS" ? (ua(e.source_crs, a), e.transformation && (e.transformation.method && e.transformation.method.name === "NTv2" ? a.nadgrids = e.transformation.parameters[0].value : a.datum_params = e.transformation.parameters.map((t) => t.value)), a) : (Object.keys(e).forEach((t) => {
    const s = e[t];
    if (s !== null)
      switch (t) {
        case "name":
        ...
        case "coordinate_system":
          if (s.axis) {
            ...
            if (i.every(Boolean) && (a.axis = i.join(""), a.axis.length === 2 && (a.axis += "u")), s.unit) {
              const { units: d, to_meter: o } = wt(s.unit);
              a.units = d, a.to_meter = o;
            } else if (s.axis[0] && s.axis[0].unit) {
              const { units: d, to_meter: o } = wt(s.axis[0].unit);
              a.units = d, a.to_meter = o;
            }
          }
          break;
        ...
        case "unit":
          ...
          break;
        case "base_crs":
          ua(s, a), a.datumCode = s.id ? s.id.authority + "_" + s.id.code : s.name;
          break;
      }
...
function $t(e) {
  for (var a = Object.keys(e), t = 0, s = a.length; t < s; ++t) {
    var n = a[t];
    en.indexOf(n) !== -1 && tn(e[n]), typeof e[n] == "object" && $t(e[n]);
  }
}
function tn(e) {
  if (e.AUTHORITY) {
  ...
  var c = function(l) {
    ...
    ["latitude_of_center", "Latitude_of_center"],
    ["lat0", "latitude_of_center", X],
    ["longitude_of_center", "Longitude_Of_Center"],
    ["longitude_of_center", "Longitude_of_center"],
    ["longc", "longitude_of_center", X],
    ["x0", "false_easting", o],
    ["y0", "false_northing", o],
    ["long0", "central_meridian", X],
    ["lat0", "latitude_of_origin", X],
    ["lat0", "standard_parallel_1", X],
    ["lat1", "standard_parallel_1", X],
    ["lat2", "standard_parallel_2", X],
    ["azimuth", "Azimuth"],
    ["alpha", "azimuth", X],
    ["srsCode", "name"]
  ];
...
function ba(e) {
  ...
  var s = t[0], n = {};
  return ue(t, n), $t(n), n[s];
}
function T(e) {
  var a = this;
  if (arguments.length === 2) {
    var t = arguments[1];
    typeof t == "string" ? t.charAt(0) === "+" ? T[
      /** @type {string} */
      e
    ] = Na(arguments[1]) : T[
      /** @type {string} */
      e
    ] = ba(arguments[1]) : t && typeof t == "object" && !("projName" in t) ? T[
      /** @type {string} */
      e
    ] = ba(arguments[1]) : (T[
      /** @type {string} */
    ...
    if (Array.isArray(e))
      return e.map(function(s) {
        return Array.isArray(s) ? T.apply(a, s) : T(s);
      });
    if (typeof e == "string") {
      if (e in T)
        return T[e];
    } else "EPSG" in e ? T["EPSG:" + e.EPSG] = e : "ESRI" in e ? T["ESRI:" + e.ESRI] = e : "IAU2000" in e ? T["IAU2000:" + e.IAU2000] = e : console.log(e);
    return;
  }
}
Ts(T);
...
function dn(e) {
  if (e.title)
    return e.title.toLowerCase().indexOf("epsg:") === 0 && _t.indexOf(e.title.substr(5)) > -1;
  var a = oe(e, "authority");
  if (a) {
    var t = oe(a, "epsg");
    return t && _t.indexOf(t) > -1;
  }
}
function cn(e) {
  var a = oe(e, "extension");
  if (a)
    return oe(a, "proj4");
}
function rn(e) {
...
function hn(e) {
  ...
  else "projName" in e ? a = e : a = ba(e);
  return a && dn(a) ? T["EPSG:3857"] : a;
}
function vt(e, a) {
  e = e || {};
  var t, s;
...
function M(e, a) {
  return a || Math.abs(e) <= I ? e : e - Ye(e) * Fe;
}
function Z(e, a, t) {
  var s = e * t, n = 0.5 * e;
...
function fn() {
  var e = this.b / this.a;
  this.es = 1 - e * e, "x0" in this || (this.x0 = 0), "y0" in this || (this.y0 = 0), this.e = Math.sqrt(this.es), this.lat_ts ? this.sphere ? this.k0 = Math.cos(this.lat_ts) : this.k0 = te(this.e, Math.sin(this.lat_ts), Math.cos(this.lat_ts)) : this.k0 || (this.k ? this.k0 = this.k : this.k0 = 1);
}
function ln(e) {
  var a = e.x, t = e.y;
  if (t * V > 90 && t * V < -90 && a * V > 180 && a * V < -180)
    return null;
  var s, n;
  if (Math.abs(Math.abs(t) - _) <= g)
    return null;
  if (this.sphere)
    s = this.x0 + this.a * this.k0 * M(a - this.long0, this.over), n = this.y0 + this.a * this.k0 * Math.log(Math.tan(P + 0.5 * t));
  else {
    var i = Math.sin(t), d = Z(this.e, t, i);
...
var un = ["Mercator", "Popular Visualisation Pseudo Mercator", "Mercator_1SP", "Mercator_Auxiliary_Sphere", "Mercator_Variant_A", "merc"], bn = {
  init: fn,
  forward: ln,
  inverse: mn,
  names: un
...
function Xt(e, a) {
  var t = be.length;
  return e.names ? (be[t] = e, e.names.forEach(function(s) {
    re[s.toLowerCase()] = t;
...
function _n(e) {
  if (!e)
    return !1;
  var a = e.toLowerCase();
  if (typeof re[a] < "u" && be[re[a]] || (a = Yt(a), a in re && be[re[a]]))
    return be[re[a]];
}
function vn() {
  pn.forEach(Xt);
...
function Mn(e, a, t, s) {
  ...
  s ? (e *= 1 - d * (Ds + d * (Fs + d * Ls)), n = e * e, d = 0) : o = Math.sqrt(d);
  var c = (n - i) / i;
  return {
    es: d,
    e: o,
...
function Bn(e, a, t, s, n) {
  if (!e) {
    var i = oe(Wt, s);
    i || (i = yn), e = i.a, a = i.b, t = i.rf;
  }
  return t && !a && (a = (1 - 1 / t) * e), (t === 0 || Math.abs(e - a) < g) && (n = !0, a = e), {
    a: e,
    b: a,
    rf: t,
    sphere: n
...
for (var Cn in ca) {
  var Ea = ca[Cn];
  Ea.datumName && (ca[Ea.datumName] = Ea);
}
function kn(e, a, t, s, n, i, d) {
  var o = {};
  return e === void 0 || e === "none" ? o.datum_type = ja : o.datum_type = Rs, a && (o.datum_params = a.map(parseFloat), (o.datum_params[0] !== 0 || o.datum_params[1] !== 0 || o.datum_params[2] !== 0) && (o.datum_type = he), o.datum_params.length > 3 && (o.datum_params[3] !== 0 || o.datum_params[4] !== 0 || o.datum_params[5] !== 0 || o.datum_params[6] !== 0) && (o.datum_type = fe, o.datum_params[3] *= Ne, o.datum_params[4] *= Ne, o.datum_params[5] *= Ne, o.datum_params[6] = o.datum_params[6] / 1e6 + 1)), d && (o.datum_type = ve, o.grids = d), o.a = t, o.b = s, o.es = n, o.ep2 = i, o;
}
var $a = {};
function Sn(e, a, t) {
  return a instanceof ArrayBuffer ? En(e, a, t) : { ready: Pn(e, a) };
}
function En(e, a, t) {
  var s = !0;
  t !== void 0 && t.includeErrorFields === !1 && (s = !1);
  var n = new DataView(a), i = Gn(n), d = Un(n, i), o = In(n, d, i, s), c = { header: d, subgrids: o };
  return $a[e] = c, c;
}
async function Pn(e, a) {
  for (var t = [], s = await a.getImageCount(), n = s - 1; n >= 0; n--) {
    var i = await a.getImage(n), d = await i.readRasters(), o = d, c = [i.getWidth(), i.getHeight()], r = i.getBoundingBox().map(yt), l = [i.fileDirectory.ModelPixelScale[0], i.fileDirectory.ModelPixelScale[1]].map(yt), f = r[0] + (c[0] - 1) * l[0], h = r[3] - (c[1] - 1) * l[1], u = o[0], m = o[1], b = [];
    for (let v = c[1] - 1; v >= 0; v--)
      for (let C = c[0] - 1; C >= 0; C--) {
        var w = v * c[0] + C;
        b.push([-ne(m[w]), ne(u[w])]);
      }
    t.push({
      del: l,
      lim: c,
...
function In(e, a, t, s) {
  for (var n = 176, i = [], d = 0; d < a.nSubgrids; d++) {
    var o = Nn(e, n, t), c = Tn(e, n, o, t, s), r = Math.round(
      1 + (o.upperLongitude - o.lowerLongitude) / o.longitudeInterval
    ), l = Math.round(
      1 + (o.upperLatitude - o.lowerLatitude) / o.latitudeInterval
    );
    i.push({
      ll: [ne(o.lowerLongitude), ne(o.lowerLatitude)],
      del: [ne(o.longitudeInterval), ne(o.latitudeInterval)],
      lim: [r, l],
...
function jn(e) {
  return e.map(function(a) {
    return [ne(a.longitudeShift), ne(a.latitudeShift)];
  });
}
function Nn(e, a, t) {
  return {
    name: Ta(e, a + 8, a + 16).trim(),
    parent: Ta(e, a + 24, a + 24 + 8).trim(),
...
function W(e, a) {
  ...
  var i = Bn(t.a, t.b, t.rf, t.ellps, t.sphere), d = Mn(i.a, i.b, i.rf, t.R_A), o = An(t.nadgrids), c = t.datum || kn(
    t.datumCode,
    t.datum_params,
    i.a,
    i.b,
  ...
  vt(this, t), vt(this, s), this.a = i.a, this.b = i.b, this.rf = i.rf, this.sphere = i.sphere, this.es = d.es, this.e = d.e, this.ep2 = d.ep2, this.datum = c, "init" in this && typeof this.init == "function" && this.init(), a(null, this);
}
W.projections = gn;
W.projections.start();
function Rn(e, a) {
  return e.datum_type !== a.datum_type || e.a !== a.a || Math.abs(e.es - a.es) > 5e-11 ? !1 : e.datum_type === he ? e.datum_params[0] === a.datum_params[0] && e.datum_params[1] === a.datum_params[1] && e.datum_params[2] === a.datum_params[2] : e.datum_type === fe ? e.datum_params[0] === a.datum_params[0] && e.datum_params[1] === a.datum_params[1] && e.datum_params[2] === a.datum_params[2] && e.datum_params[3] === a.datum_params[3] && e.datum_params[4] === a.datum_params[4] && e.datum_params[5] === a.datum_params[5] && e.datum_params[6] === a.datum_params[6] : !0;
}
function Jt(e, a, t) {
...
function Qt(e, a, t, s) {
  var n = 1e-12, i = n * n, d = 30, o, c, r, l, f, h, u, m, b, w, p, v, C, k = e.x, y = e.y, B = e.z ? e.z : 0, S, x, E;
  if (o = Math.sqrt(k * k + y * y), c = Math.sqrt(k * k + y * y + B * B), o / t < n) {
    if (S = 0, c / t < n)
      return x = _, E = -s, {
        x: e.x,
        y: e.y,
  ...
  while (v * v > i && C < d);
  return x = Math.atan(p / Math.abs(w)), {
    x: S,
    y: x,
    z: E
  };
...
function Fn(e, a, t) {
  if (Rn(e, a) || e.datum_type === ja || a.datum_type === ja)
    return t;
  var s = e.a, n = e.es;
  if (e.datum_type === ve) {
    var i = Mt(e, !1, t);
  ...
  if (a.datum_type === ve && (d = mt, o = Os, c = ut), n === c && s === d && !sa(e.datum_type) && !sa(a.datum_type))
    return t;
  if (t = Jt(t, n, s), sa(e.datum_type) && (t = On(t, e.datum_type, e.datum_params)), sa(a.datum_type) && (t = Dn(t, a.datum_type, a.datum_params)), t = Qt(t, c, d, o), a.datum_type === ve) {
    var r = Mt(a, !0, t);
    if (r !== 0)
      return;
  }
  ...
  e:
    for (var d = 0; d < e.grids.length; d++) {
      ...
      for (var c = o.grid.subgrids, r = 0, l = c.length; r < l; r++) {
        ...
        if (!(m > s.y || u > s.x || w < s.y || b < s.x) && (n = Ln(s, a, f), !isNaN(n.x)))
          break e;
      }
    }
  return isNaN(n.x) ? (console.log("Failed to find a grid shift table for location '" + -s.x * V + " " + s.y * V + " tried: '" + i + "'"), -1) : (t.x = -n.x, t.y = n.y, 0);
}
function Ln(e, a, t) {
...
function Vn(e, a) {
  return (e.datum.datum_type === he || e.datum.datum_type === fe || e.datum.datum_type === ve) && a.datumCode !== "WGS84" || (a.datum.datum_type === he || a.datum.datum_type === fe || a.datum.datum_type === ve) && e.datumCode !== "WGS84";
}
function wa(e, a, t, s) {
  var n;
  Array.isArray(t) ? t = Zt(t) : t = {
    x: t.x,
    y: t.y,
    z: t.z,
    m: t.m
  ...
  var i = t.z !== void 0;
  if (qn(t), e.datum && a.datum && Vn(e, a) && (n = new W("WGS84"), t = wa(e, n, t, s), e = n), s && e.axis !== "enu" && (t = Ct(e, !1, t)), e.projName === "longlat")
    t = {
      x: t.x * U,
      y: t.y * U,
      z: t.z || 0
    ...
    return;
  if (e.from_greenwich && (t.x += e.from_greenwich), t = Fn(e.datum, a.datum, t), !!t)
    ...
    t, a.from_greenwich && (t = {
      ...
      y: t.y / a.to_meter,
      z: t.z || 0
    })), s && a.axis !== "enu" ? Ct(a, !0, t) : (t && !i && a.projName !== "geocent" && delete t.z, t);
}
var St = W("WGS84");
function Pa(e, a, t, s) {
  var n, i, d;
  return Array.isArray(t) ? (n = wa(e, a, t, s) || { x: NaN, y: NaN }, t.length > 2 ? typeof e.name < "u" && e.name === "geocent" || typeof a.name < "u" && a.name === "geocent" ? typeof n.z == "number" ? (
    /** @type {T} */
    ...
    /** @type {T} */
    [n.x, n.y]
  )) : (i = wa(e, a, t, s), d = Object.keys(t), d.length === 2 || d.forEach(function(o) {
...
function na(e) {
  return e instanceof W ? e : typeof e == "object" && "oProj" in e ? e.oProj : W(
...
function Hn(e, a, t) {
  var s, n, i = !1, d;
  return typeof a > "u" ? (n = na(e), s = St, i = !0) : (typeof /** @type {?} */
  a.x < "u" || Array.isArray(a)) && (t = /** @type {T} */
  /** @type {?} */
  a, n = na(e), s = St, i = !0), s || (s = na(e)), n || (n = na(
...
var Et = 6, es = "AJSAJS", as = "AFAFAF", we = 65, L = 73, Y = 79, Ge = 86, Ue = 90, $n = {
  forward: ts,
  inverse: Kn,
  toPoint: ss
};
...
function Xn(e) {
  var a = e.lat, t = e.lon, s = 6378137, n = 669438e-8, i = 0.9996, d, o, c, r, l, f, h, u = Aa(a), m = Aa(t), b, w;
  w = Math.floor((t + 180) / 6) + 1, t === 180 && (w = 60), a >= 56 && a < 64 && t >= 3 && t < 12 && (w = 32), a >= 72 && a < 84 && (t >= 0 && t < 9 ? w = 31 : t >= 9 && t < 21 ? w = 33 : t >= 21 && t < 33 ? w = 35 : t >= 33 && t < 42 && (w = 37)), d = (w - 1) * 6 - 180 + 3, b = Aa(d), o = n / (1 - n), c = s / Math.sqrt(1 - n * Math.sin(u) * Math.sin(u)), r = Math.tan(u) * Math.tan(u), l = o * Math.cos(u) * Math.cos(u), f = Math.cos(u) * (m - b), h = s * ((1 - n / 4 - 3 * n * n / 64 - 5 * n * n * n / 256) * u - (3 * n / 8 + 3 * n * n / 32 + 45 * n * n * n / 1024) * Math.sin(2 * u) + (15 * n * n / 256 + 45 * n * n * n / 1024) * Math.sin(4 * u) - 35 * n * n * n / 3072 * Math.sin(6 * u));
...
function Ka(e) {
  ...
  var y = v - l * Math.tan(v) / u * (m * m / 2 - (5 + 3 * f + 10 * h - 4 * h * h - 9 * c) * m * m * m * m / 24 + (61 + 90 * f + 298 * h + 45 * f * f - 252 * c - 3 * h * h) * m * m * m * m * m * m / 720);
  y = Pt(y);
...
function Wn(e, a) {
  ...
  return e.zoneNumber + e.zoneLetter + Jn(e.easting, e.northing, e.zoneNumber) + t.substr(t.length - 5, a) + s.substr(s.length - 5, a);
}
function Jn(e, a, t) {
  var s = ns(t), n = Math.floor(e / 1e5), i = Math.floor(a / 1e5) % 20;
  return Qn(n, i, s);
}
function ns(e) {
  var a = e % Et;
...
function Qn(e, a, t) {
  var s = t - 1, n = es.charCodeAt(s), i = as.charCodeAt(s), d = n + e - 1, o = i + a, c = !1;
  d > Ue && (d = d - Ue + we - 1, c = !0), (d === L || n < L && d > L || (d > L || n < L) && c) && d++, (d === Y || n < Y && d > Y || (d > Y || n < Y) && c) && (d++, d === L && d++), d > Ue && (d = d - Ue + we - 1), o > Ge ? (o = o - Ge + we - 1, c = !0) : c = !1, (o === L || i < L && o > L || (o > L || i < L) && c) && o++, (o === Y || i < Y && o > Y || (o > Y || i < Y) && c) && (o++, o === L && o++), o > Ge && (o = o - Ge + we - 1);
...
function is(e) {
  ...
  t = e.substring(i, i += 2);
  for (var c = ns(d), r = Zn(t.charAt(0), c), l = e1(t.charAt(1), c); l < a1(o); )
    l += 2e6;
  var f = a - i;
  if (f % 2 !== 0)
...
function Zn(e, a) {
  for (var t = es.charCodeAt(a - 1), s = 1e5, n = !1; t !== e.charCodeAt(0); ) {
    if (t++, t === L && t++, t === Y && t++, t > Ue) {
      if (n)
...
function e1(e, a) {
  ...
  for (var t = as.charCodeAt(a - 1), s = 0, n = !1; t !== e.charCodeAt(0); ) {
    if (t++, t === L && t++, t === Y && t++, t > Ge) {
      if (n)
        throw "Bad character: " + e;
      t = we, n = !0;
    }
...
Me.fromMGRS = function(e) {
  return new Me(ss(e));
};
Me.prototype.toMGRS = function(e) {
  return ts([this.x, this.y], e);
};
var t1 = 1, s1 = 0.25, At = 0.046875, xt = 0.01953125, Gt = 0.01068115234375, n1 = 0.75, i1 = 0.46875, o1 = 0.013020833333333334, d1 = 0.007120768229166667, c1 = 0.3645833333333333, r1 = 0.005696614583333333, h1 = 0.3076171875;
function Xa(e) {
  var a = [];
  a[0] = t1 - e * (s1 + e * (At + e * (xt + e * Gt))), a[1] = e * (n1 - e * (At + e * (xt + e * Gt)));
  var t = e * e;
  return a[2] = t * (i1 - e * (o1 + e * d1)), t *= e, a[3] = t * (c1 - e * r1), a[4] = t * e * h1, a;
}
function Be(e, a, t, s) {
...
var f1 = 20;
function Ya(e, a, t) {
  for (var s = 1 / (1 - a), n = e, i = f1; i; --i) {
    var d = Math.sin(n), o = 1 - a * d * d;
    if (o = (Be(n, d, Math.cos(n), t) - e) * (o * Math.sqrt(o)) * s, n -= o, Math.abs(o) < g)
      return n;
  }
  return n;
}
function l1() {
  this.x0 = this.x0 !== void 0 ? this.x0 : 0, this.y0 = this.y0 !== void 0 ? this.y0 : 0, this.long0 = this.long0 !== void 0 ? this.long0 : 0, this.lat0 = this.lat0 !== void 0 ? this.lat0 : 0, this.es && (this.en = Xa(this.es), this.ml0 = Be(this.lat0, Math.sin(this.lat0), Math.cos(this.lat0), this.en));
}
function m1(e) {
  var a = e.x, t = e.y, s = M(a - this.long0, this.over), n, i, d, o = Math.sin(t), c = Math.cos(t);
  if (this.es) {
    var l = c * s, f = Math.pow(l, 2), h = this.ep2 * Math.pow(c, 2), u = Math.pow(h, 2), m = Math.abs(c) > g ? Math.tan(t) : 0, b = Math.pow(m, 2), w = Math.pow(b, 2);
    n = 1 - this.es * Math.pow(o, 2), l = l / Math.sqrt(n);
    var p = Be(t, o, c, this.en);
...
function u1(e) {
  ...
  if (this.es)
    if (a = this.ml0 + d / this.k0, t = Ya(a, this.es, this.en), Math.abs(t) < _) {
      var f = Math.sin(t), h = Math.cos(t), u = Math.abs(h) > g ? Math.tan(t) : 0, m = this.ep2 * Math.pow(h, 2), b = Math.pow(m, 2), w = Math.pow(u, 2), p = Math.pow(w, 2);
...
var b1 = ["Fast_Transverse_Mercator", "Fast Transverse Mercator"], ra = {
  init: l1,
  forward: m1,
  inverse: u1,
  names: b1
...
function z1(e) {
  var a = Math.abs(e);
  return a = w1(a * (1 + a / (q(1, a) + 1))), e < 0 ? -a : a;
}
function Wa(e, a) {
...
function ds(e, a, t) {
  for (var s = Math.sin(a), n = Math.cos(a), i = os(t), d = _1(t), o = 2 * n * d, c = -2 * s * i, r = e.length - 1, l = e[r], f = 0, h = 0, u = 0, m, b; --r >= 0; )
...
function v1() {
  ...
  this.approx && (ra.init.apply(this), this.forward = ra.forward, this.inverse = ra.inverse), this.x0 = this.x0 !== void 0 ? this.x0 : 0, this.y0 = this.y0 !== void 0 ? this.y0 : 0, this.long0 = this.long0 !== void 0 ? this.long0 : 0, this.lat0 = this.lat0 !== void 0 ? this.lat0 : 0, this.cgb = [], this.cbg = [], this.utg = [], this.gtu = [];
  ...
  var s = Wa(this.cbg, this.lat0);
  this.Zb = -this.Qn * (s + p1(this.gtu, 2 * s));
}
function g1(e) {
  var a = M(e.x - this.long0, this.over), t = e.y;
  t = Wa(this.cbg, t);
  var s = Math.sin(t), n = Math.cos(t), i = Math.sin(a), d = Math.cos(a);
  t = Math.atan2(s, d * n), a = Math.atan2(i * n, q(s, n * d)), a = z1(Math.tan(a));
  var o = ds(this.gtu, 2 * t, 2 * a);
  t = t + o[0], a = a + o[1];
  var c, r;
...
function y1(e) {
  ...
  if (Math.abs(a) <= 2.623395162778) {
    var i = ds(this.utg, 2 * t, 2 * a);
    t = t + i[0], a = a + i[1], a = Math.atan(os(a));
    var d = Math.sin(t), o = Math.cos(t), c = Math.sin(a), r = Math.cos(a);
    t = Math.atan2(d * r, q(c, r * o)), a = Math.atan2(c, r * o), s = M(a + this.long0, this.over), n = Wa(this.cgb, t);
  } else
    s = 1 / 0, n = 1 / 0;
  return e.x = s, e.y = n, e;
}
var M1 = ["Extended_Transverse_Mercator", "Extended Transverse Mercator", "etmerc", "Transverse_Mercator", "Transverse Mercator", "Gauss Kruger", "Gauss_Kruger", "tmerc"], ha = {
  init: v1,
  forward: g1,
  inverse: y1,
  names: M1
...
var C1 = "etmerc";
function k1() {
  var e = B1(this.zone, this.long0);
  if (e === void 0)
    throw new Error("unknown utm zone");
  this.lat0 = 0, this.long0 = (6 * Math.abs(e) - 183) * U, this.x0 = 5e5, this.y0 = this.utmSouth ? 1e7 : 0, this.k0 = 0.9996, ha.init.apply(this), this.forward = ha.forward, this.inverse = ha.inverse;
}
var S1 = ["Universal Transverse Mercator System", "utm"], E1 = {
  init: k1,
  names: S1,
  dependsOn: C1
};
...
function A1() {
  ...
  a *= a, this.rc = Math.sqrt(1 - this.es) / (1 - this.es * e * e), this.C = Math.sqrt(1 + this.es * a * a / (1 - this.es)), this.phic0 = Math.asin(e / this.C), this.ratexp = 0.5 * this.C * this.e, this.K = Math.tan(0.5 * this.phic0 + P) / (Math.pow(Math.tan(0.5 * this.lat0 + P), this.C) * Ja(this.e * e, this.ratexp));
}
function x1(e) {
  var a = e.x, t = e.y;
  return e.y = 2 * Math.atan(this.K * Math.pow(Math.tan(0.5 * t + P), this.C) * Ja(this.e * Math.sin(t), this.ratexp)) - _, e.x = this.C * a, e;
}
function G1(e) {
  for (var a = 1e-14, t = e.x / this.C, s = e.y, n = Math.pow(Math.tan(0.5 * s + P) / this.K, 1 / this.C), i = P1; i > 0 && (s = 2 * Math.atan(n * Ja(this.e * Math.sin(e.y), -0.5 * this.e)) - _, !(Math.abs(s - e.y) < a)); --i)
    e.y = s;
...
function I1(e) {
  var a, t, s, n;
  return e.x = M(e.x - this.long0, this.over), Qa.forward.apply(this, [e]), a = Math.sin(e.y), t = Math.cos(e.y), s = Math.cos(e.x), n = this.k0 * this.R2 / (1 + this.sinc0 * a + this.cosc0 * t * s), e.x = n * t * Math.sin(e.x), e.y = n * (this.cosc0 * a - this.sinc0 * t * s), e.x = this.a * e.x + this.x0, e.y = this.a * e.y + this.y0, e;
}
function j1(e) {
  var a, t, s, n, i;
  if (e.x = (e.x - this.x0) / this.a, e.y = (e.y - this.y0) / this.a, e.x /= this.k0, e.y /= this.k0, i = q(e.x, e.y)) {
    ...
    n = this.phic0, s = 0;
  return e.x = s, e.y = n, Qa.inverse.apply(this, [e]), e.x = M(e.x + this.long0, this.over), e;
}
var N1 = ["Stereographic_North_Pole", "Oblique_Stereographic", "sterea", "Oblique Stereographic Alternative", "Double_Stereographic"], T1 = {
  init: U1,
  forward: I1,
  inverse: j1,
  names: N1
...
function R1() {
  this.x0 = this.x0 || 0, this.y0 = this.y0 || 0, this.lat0 = this.lat0 || 0, this.long0 = this.long0 || 0, this.coslat0 = Math.cos(this.lat0), this.sinlat0 = Math.sin(this.lat0), this.sphere ? this.k0 === 1 && !isNaN(this.lat_ts) && Math.abs(this.coslat0) <= g && (this.k0 = 0.5 * (1 + Ye(this.lat0) * Math.sin(this.lat_ts))) : (Math.abs(this.coslat0) <= g && (this.lat0 > 0 ? this.con = 1 : this.con = -1), this.cons = Math.sqrt(Math.pow(1 + this.e, 1 + this.e) * Math.pow(1 - this.e, 1 - this.e)), this.k0 === 1 && !isNaN(this.lat_ts) && Math.abs(this.coslat0) <= g && Math.abs(Math.cos(this.lat_ts)) > g && (this.k0 = 0.5 * this.cons * te(this.e, Math.sin(this.lat_ts), Math.cos(this.lat_ts)) / Z(this.e, this.con * this.lat_ts, this.con * Math.sin(this.lat_ts))), this.ms1 = te(this.e, this.sinlat0, this.coslat0), this.X0 = 2 * Math.atan(Za(this.lat0, this.sinlat0, this.e)) - _, this.cosX0 = Math.cos(this.X0), this.sinX0 = Math.sin(this.X0));
}
function O1(e) {
  var a = e.x, t = e.y, s = Math.sin(t), n = Math.cos(t), i, d, o, c, r, l, f = M(a - this.long0, this.over);
  return Math.abs(Math.abs(a - this.long0) - Math.PI) <= g && Math.abs(t + this.lat0) <= g ? (e.x = NaN, e.y = NaN, e) : this.sphere ? (i = 2 * this.k0 / (1 + this.sinlat0 * s + this.coslat0 * n * Math.cos(f)), e.x = this.a * i * n * Math.sin(f) + this.x0, e.y = this.a * i * (this.coslat0 * s - this.sinlat0 * n * Math.cos(f)) + this.y0, e) : (d = 2 * Math.atan(Za(t, s, this.e)) - _, c = Math.cos(d), o = Math.sin(d), Math.abs(this.coslat0) <= g ? (r = Z(this.e, t * this.con, this.con * s), l = 2 * this.a * this.k0 * r / this.cons, e.x = this.x0 + l * Math.sin(a - this.long0), e.y = this.y0 - this.con * l * Math.cos(a - this.long0), e) : (Math.abs(this.sinlat0) < g ? (i = 2 * this.a * this.k0 / (1 + c * Math.cos(f)), e.y = i * o) : (i = 2 * this.a * this.k0 * this.ms1 / (this.cosX0 * (1 + this.sinX0 * o + this.cosX0 * c * Math.cos(f))), e.y = i * (this.cosX0 * o - this.sinX0 * c * Math.cos(f)) + this.y0), e.x = i * c * Math.sin(f) + this.x0, e));
}
function D1(e) {
  ...
  if (this.sphere) {
    ...
    return a = this.long0, t = this.lat0, d <= g ? (e.x = a, e.y = t, e) : (t = Math.asin(Math.cos(o) * this.sinlat0 + e.y * Math.sin(o) * this.coslat0 / d), Math.abs(this.coslat0) < g ? this.lat0 > 0 ? a = M(this.long0 + Math.atan2(e.x, -1 * e.y), this.over) : a = M(this.long0 + Math.atan2(e.x, e.y), this.over) : a = M(this.long0 + Math.atan2(e.x * Math.sin(o), d * this.coslat0 * Math.cos(o) - e.y * this.sinlat0 * Math.sin(o)), this.over), e.x = a, e.y = t, e);
    ...
    n = 2 * Math.atan(d * this.cosX0 / (2 * this.a * this.k0 * this.ms1)), a = this.long0, d <= g ? i = this.X0 : (i = Math.asin(Math.cos(n) * this.sinX0 + e.y * Math.sin(n) * this.cosX0 / d), a = M(this.long0 + Math.atan2(e.x * Math.sin(n), d * this.cosX0 * Math.cos(n) - e.y * this.sinX0 * Math.sin(n)), this.over)), t = -1 * qe(this.e, Math.tan(0.5 * (_ + i)));
  return e.x = a, e.y = t, e;
}
var F1 = ["stere", "Stereographic_South_Pole", "Polar_Stereographic_variant_A", "Polar_Stereographic_variant_B", "Polar_Stereographic"], L1 = {
  init: R1,
  forward: O1,
  inverse: D1,
  names: F1,
...
function X1(e) {
  ...
  return "no_uoff" in e || "no_off" in e || a.indexOf(t) !== -1 || a.indexOf(Yt(t)) !== -1;
}
function Y1() {
  var e, a, t, s, n, i, d, o, c, r, l = 0, f, h = 0, u = 0, m = 0, b = 0, w = 0, p = 0;
  this.no_off = X1(this), this.no_rot = "no_rot" in this;
  var v = !1;
  "alpha" in this && (v = !0);
  var C = !1;
  if ("rectified_grid_angle" in this && (C = !0), v && (p = this.alpha), C && (l = this.rectified_grid_angle), v || C)
    h = this.longc;
  else if (u = this.long1, b = this.lat1, m = this.long2, w = this.lat2, Math.abs(b - w) <= me || (e = Math.abs(b)) <= me || Math.abs(e - _) <= me || Math.abs(Math.abs(this.lat0) - _) <= me || Math.abs(Math.abs(w) - _) <= me)
    throw new Error();
  var k = 1 - this.es;
  a = Math.sqrt(k), Math.abs(this.lat0) > g ? (o = Math.sin(this.lat0), t = Math.cos(this.lat0), e = 1 - this.es * o * o, this.B = t * t, this.B = Math.sqrt(1 + this.es * this.B * this.B / k), this.A = this.B * this.k0 * a / e, s = this.B * a / (t * Math.sqrt(e)), n = s * s - 1, n <= 0 ? n = 0 : (n = Math.sqrt(n), this.lat0 < 0 && (n = -n)), this.E = n += s, this.E *= Math.pow(Z(this.e, this.lat0, o), this.B)) : (this.B = 1 / a, this.A = this.k0, this.E = s = n = 1), v || C ? (v ? (f = Math.asin(Math.sin(p) / s), C || (l = p)) : (f = l, p = Math.asin(s * Math.sin(f))), this.lam0 = h - Math.asin(0.5 * (n - 1 / n) * Math.tan(f)) / this.B) : (i = Math.pow(Z(this.e, b, Math.sin(b)), this.B), d = Math.pow(Z(this.e, w, Math.sin(w)), this.B), n = this.E / i, c = (d - i) / (d + i), r = this.E * this.E, r = (r - d * i) / (r + d * i), e = u - m, e < -Math.PI ? m -= Fe : e > Math.PI && (m += Fe), this.lam0 = M(0.5 * (u + m) - Math.atan(r * Math.tan(0.5 * this.B * (u - m)) / c) / this.B, this.over), f = Math.atan(2 * Math.sin(this.B * M(u - this.lam0, this.over)) / (n - 1 / n)), l = p = Math.asin(s * Math.sin(f))), this.singam = Math.sin(f), this.cosgam = Math.cos(f), this.sinrot = Math.sin(l), this.cosrot = Math.cos(l), this.rB = 1 / this.B, this.ArB = this.A * this.rB, this.BrA = 1 / this.ArB, this.no_off ? this.u_0 = 0 : (this.u_0 = Math.abs(this.ArB * Math.atan(Math.sqrt(s * s - 1) / Math.cos(p))), this.lat0 < 0 && (this.u_0 = -this.u_0)), n = 0.5 * f, this.v_pole_n = this.ArB * Math.log(Math.tan(P - n)), this.v_pole_s = this.ArB * Math.log(Math.tan(P + n));
}
function W1(e) {
  var a = {}, t, s, n, i, d, o, c, r;
  if (e.x = e.x - this.lam0, Math.abs(Math.abs(e.y) - _) > g) {
    if (d = this.E / Math.pow(Z(this.e, e.y, Math.sin(e.y)), this.B), o = 1 / d, t = 0.5 * (d - o), s = 0.5 * (d + o), i = Math.sin(this.B * e.x), n = (t * this.singam - i * this.cosgam) / s, Math.abs(Math.abs(n) - 1) < g)
      throw new Error();
    r = 0.5 * this.ArB * Math.log((1 - n) / (1 + n)), o = Math.cos(this.B * e.x), Math.abs(o) < me ? c = this.A * e.x : c = this.ArB * Math.atan2(t * this.cosgam + i * this.singam, o);
  } else
...
function J1(e) {
  var a, t, s, n, i, d, o, c = {};
  if (e.x = (e.x - this.x0) * (1 / this.a), e.y = (e.y - this.y0) * (1 / this.a), this.no_rot ? (t = e.y, a = e.x) : (t = e.x * this.cosrot - e.y * this.sinrot, a = e.y * this.cosrot + e.x * this.sinrot + this.u_0), s = Math.exp(-this.BrA * t), n = 0.5 * (s - 1 / s), i = 0.5 * (s + 1 / s), d = Math.sin(this.BrA * a), o = (d * this.cosgam + n * this.singam) / i, Math.abs(Math.abs(o) - 1) < g)
    c.x = 0, c.y = o < 0 ? -_ : _;
  else {
...
var Q1 = ["Hotine_Oblique_Mercator", "Hotine Oblique Mercator", "Hotine_Oblique_Mercator_variant_A", "Hotine_Oblique_Mercator_Variant_B", "Hotine_Oblique_Mercator_Azimuth_Natural_Origin", "Hotine_Oblique_Mercator_Two_Point_Natural_Origin", "Hotine_Oblique_Mercator_Azimuth_Center", "Oblique_Mercator", "omerc"], Z1 = {
  init: Y1,
  forward: W1,
  inverse: J1,
  names: Q1
...
function e2() {
  if (this.lat2 || (this.lat2 = this.lat1), this.k0 || (this.k0 = 1), this.x0 = this.x0 || 0, this.y0 = this.y0 || 0, !(Math.abs(this.lat1 + this.lat2) < g)) {
    ...
    var a = Math.sin(this.lat1), t = Math.cos(this.lat1), s = te(this.e, a, t), n = Z(this.e, this.lat1, a), i = Math.sin(this.lat2), d = Math.cos(this.lat2), o = te(this.e, i, d), c = Z(this.e, this.lat2, i), r = Math.abs(Math.abs(this.lat0) - _) < g ? 0 : Z(this.e, this.lat0, Math.sin(this.lat0));
    Math.abs(this.lat1 - this.lat2) > g ? this.ns = Math.log(s / o) / Math.log(n / c) : this.ns = a, isNaN(this.ns) && (this.ns = a), this.f0 = s / (this.ns * Math.pow(n, this.ns)), this.rh = this.a * this.f0 * Math.pow(r, this.ns), this.title || (this.title = "Lambert Conformal Conic");
  }
}
function a2(e) {
  var a = e.x, t = e.y;
  Math.abs(2 * Math.abs(t) - Math.PI) <= g && (t = Ye(t) * (_ - 2 * g));
  var s = Math.abs(Math.abs(t) - _), n, i;
  if (s > g)
    n = Z(this.e, t, Math.sin(t)), i = this.a * this.f0 * Math.pow(n, this.ns);
  else {
    if (s = t * this.ns, s <= 0)
      return null;
    i = 0;
...
var c2 = ["Krovak", "Krovak Modified", "Krovak (North Orientated)", "Krovak Modified (North Orientated)", "krovak"], r2 = {
  init: i2,
  forward: o2,
  inverse: d2,
  names: c2
...
function ce(e) {
  return Math.abs(e) < _ ? e : e - Ye(e) * Math.PI;
}
function za(e, a, t, s, n) {
  var i, d;
  i = e / a;
  for (var o = 0; o < 15; o++)
...
function h2() {
  this.sphere || (this.e0 = We(this.es), this.e1 = Je(this.es), this.e2 = Qe(this.es), this.e3 = Ze(this.es), this.ml0 = this.a * D(this.e0, this.e1, this.e2, this.e3, this.lat0));
}
function f2(e) {
  var a, t, s = e.x, n = e.y;
  if (s = M(s - this.long0, this.over), this.sphere)
    ...
    var i = Math.sin(n), d = Math.cos(n), o = et(this.a, this.e, i), c = Math.tan(n) * Math.tan(n), r = s * Math.cos(n), l = r * r, f = this.es * d * d / (1 - this.es), h = this.a * D(this.e0, this.e1, this.e2, this.e3, n);
...
function l2(e) {
  ...
  if (this.sphere) {
    ...
    var d = this.ml0 / this.a + t, o = za(d, this.e0, this.e1, this.e2, this.e3);
    if (Math.abs(Math.abs(o) - _) <= g)
      return e.x = this.long0, e.y = _, t < 0 && (e.y *= -1), e;
    var c = et(this.a, this.e, Math.sin(o)), r = c * c * c / this.a / this.a * (1 - this.es), l = Math.pow(Math.tan(o), 2), f = a * this.a / c, h = f * f;
    s = o - c * Math.tan(o) / r * f * f * (0.5 - (1 + 3 * l) * f * f / 24), n = f * (1 - h * (l / 3 + (1 + 3 * l) * l * h / 15)) / Math.cos(o);
  }
  return e.x = M(n + this.long0, this.over), e.y = ce(s), e;
}
var m2 = ["Cassini", "Cassini_Soldner", "cass"], u2 = {
  init: h2,
  forward: f2,
  inverse: l2,
  names: m2
...
var Ra = 1, Oa = 2, Da = 3, fa = 4;
function b2() {
  var e = Math.abs(this.lat0);
  if (Math.abs(e - _) < g ? this.mode = this.lat0 < 0 ? Ra : Oa : Math.abs(e) < g ? this.mode = Da : this.mode = fa, this.es > 0) {
    var a;
    switch (this.qp = ie(this.e, 1), this.mmf = 0.5 / (1 - this.es), this.apa = B2(this.es), this.mode) {
      case Oa:
        this.dd = 1;
        break;
      case Ra:
      ...
      case Da:
        ...
        break;
      case fa:
        this.rq = Math.sqrt(0.5 * this.qp), a = Math.sin(this.lat0), this.sinb1 = ie(this.e, a) / this.qp, this.cosb1 = Math.sqrt(1 - this.sinb1 * this.sinb1), this.dd = Math.cos(this.lat0) / (Math.sqrt(1 - this.es * a * a) * this.rq * this.cosb1), this.ymf = (this.xmf = this.rq) / this.dd, this.xmf *= this.dd;
        break;
    }
  } else
    this.mode === fa && (this.sinph0 = Math.sin(this.lat0), this.cosph0 = Math.cos(this.lat0));
}
function w2(e) {
  ...
  if (f = M(f - this.long0, this.over), this.sphere) {
    if (i = Math.sin(h), l = Math.cos(h), s = Math.cos(f), this.mode === this.OBLIQ || this.mode === this.EQUIT) {
      if (t = this.mode === this.EQUIT ? 1 + l * s : 1 + this.sinph0 * i + this.cosph0 * l * s, t <= g)
        return null;
      t = Math.sqrt(2 / t), a = t * l * Math.sin(f), t *= this.mode === this.EQUIT ? i : this.cosph0 * i - this.sinph0 * l * s;
    } else if (this.mode === this.N_POLE || this.mode === this.S_POLE) {
      if (this.mode === this.N_POLE && (s = -s), Math.abs(h + this.lat0) < g)
        return null;
      t = P - h * 0.5, t = 2 * (this.mode === this.S_POLE ? Math.cos(t) : Math.sin(t)), a = t * Math.sin(f), t *= s;
    }
  } else {
    switch (o = 0, c = 0, r = 0, s = Math.cos(f), n = Math.sin(f), i = Math.sin(h), d = ie(this.e, i), (this.mode === this.OBLIQ || this.mode === this.EQUIT) && (o = d / this.qp, c = Math.sqrt(1 - o * o)), this.mode) {
      case this.OBLIQ:
...
function z2(e) {
  ...
  if (this.sphere) {
    ...
    switch (n = 2 * Math.asin(n), (this.mode === this.OBLIQ || this.mode === this.EQUIT) && (h = Math.sin(n), l = Math.cos(n)), this.mode) {
      case this.EQUIT:
        n = Math.abs(f) <= g ? 0 : Math.asin(t * h / f), a *= h, t = l * f;
        break;
      case this.OBLIQ:
        n = Math.abs(f) <= g ? this.lat0 : Math.asin(l * this.sinph0 + t * h * this.cosph0 / f), a *= h * this.cosph0, t = (l - Math.sin(n) * this.sinph0) * f;
        break;
      case this.N_POLE:
        t = -t, n = _ - n;
        break;
    ...
    if (r = 0, this.mode === this.OBLIQ || this.mode === this.EQUIT) {
      if (a /= this.dd, t *= this.dd, c = Math.sqrt(a * a + t * t), c < g)
    ...
    s = Math.atan2(a, t), n = C2(Math.asin(r), this.apa);
  }
  return e.x = M(this.long0 + s, this.over), e.y = n, e;
}
var p2 = 0.3333333333333333, _2 = 0.17222222222222222, v2 = 0.10257936507936508, g2 = 0.06388888888888888, y2 = 0.0664021164021164, M2 = 0.016415012942191543;
function B2(e) {
  var a, t = [];
  return t[0] = e * p2, a = e * e, t[0] += a * _2, t[1] = a * g2, a *= e, t[0] += a * v2, t[1] += a * y2, t[2] = a * M2, t;
}
function C2(e, a) {
  var t = e + e;
  return e + a[0] * Math.sin(t) + a[1] * Math.sin(t + t) + a[2] * Math.sin(t + t + t);
}
var k2 = ["Lambert Azimuthal Equal Area", "Lambert_Azimuthal_Equal_Area", "laea"], S2 = {
  init: b2,
  forward: w2,
  inverse: z2,
  names: k2,
...
function E2() {
  Math.abs(this.lat1 + this.lat2) < g || (this.temp = this.b / this.a, this.es = 1 - Math.pow(this.temp, 2), this.e3 = Math.sqrt(this.es), this.sin_po = Math.sin(this.lat1), this.cos_po = Math.cos(this.lat1), this.t1 = this.sin_po, this.con = this.sin_po, this.ms1 = te(this.e3, this.sin_po, this.cos_po), this.qs1 = ie(this.e3, this.sin_po), this.sin_po = Math.sin(this.lat2), this.cos_po = Math.cos(this.lat2), this.t2 = this.sin_po, this.ms2 = te(this.e3, this.sin_po, this.cos_po), this.qs2 = ie(this.e3, this.sin_po), this.sin_po = Math.sin(this.lat0), this.cos_po = Math.cos(this.lat0), this.t3 = this.sin_po, this.qs0 = ie(this.e3, this.sin_po), Math.abs(this.lat1 - this.lat2) > g ? this.ns0 = (this.ms1 * this.ms1 - this.ms2 * this.ms2) / (this.qs2 - this.qs1) : this.ns0 = this.con, this.c = this.ms1 * this.ms1 + this.ns0 * this.qs1, this.rh = this.a * Math.sqrt(this.c - this.ns0 * this.qs0) / this.ns0);
}
function P2(e) {
  var a = e.x, t = e.y;
  this.sin_phi = Math.sin(t), this.cos_phi = Math.cos(t);
  var s = ie(this.e3, this.sin_phi), n = this.a * Math.sqrt(this.c - this.ns0 * s) / this.ns0, i = this.ns0 * M(a - this.long0, this.over), d = n * Math.sin(i) + this.x0, o = this.rh - n * Math.cos(i) + this.y0;
...
var G2 = ["Albers_Conic_Equal_Area", "Albers_Equal_Area", "Albers", "aea"], U2 = {
  init: E2,
  forward: P2,
  inverse: A2,
  names: G2,
...
function D2() {
  this.sphere || (this.k0 = te(this.e, Math.sin(this.lat_ts), Math.cos(this.lat_ts)));
}
function F2(e) {
...
function L2(e) {
  e.x -= this.x0, e.y -= this.y0;
  var a, t;
  return this.sphere ? (a = M(this.long0 + e.x / this.a / Math.cos(this.lat_ts), this.over), t = Math.asin(e.y / this.a * Math.cos(this.lat_ts))) : (t = O2(this.e, 2 * e.y * this.k0 / this.a), a = M(this.long0 + e.x / (this.a * this.k0), this.over)), e.x = a, e.y = t, e;
}
var q2 = ["cea"], V2 = {
  init: D2,
  forward: F2,
...
function $2(e) {
  var a = e.x, t = e.y, s = M(a - this.long0, this.over), n = ce(t - this.lat0);
  return e.x = this.x0 + this.a * s * this.rc, e.y = this.y0 + this.a * n, e;
}
function K2(e) {
  var a = e.x, t = e.y;
  return e.x = M(this.long0 + (a - this.x0) / (this.a * this.rc), this.over), e.y = ce(this.lat0 + (t - this.y0) / this.a), e;
}
var X2 = ["Equirectangular", "Equidistant_Cylindrical", "Equidistant_Cylindrical_Spherical", "eqc"], Y2 = {
  init: H2,
  forward: $2,
  inverse: K2,
  names: X2
...
function W2() {
  this.temp = this.b / this.a, this.es = 1 - Math.pow(this.temp, 2), this.e = Math.sqrt(this.es), this.e0 = We(this.es), this.e1 = Je(this.es), this.e2 = Qe(this.es), this.e3 = Ze(this.es), this.ml0 = this.a * D(this.e0, this.e1, this.e2, this.e3, this.lat0);
}
function J2(e) {
  var a = e.x, t = e.y, s, n, i, d = M(a - this.long0, this.over);
  if (i = d * Math.sin(t), this.sphere)
    Math.abs(t) <= g ? (s = this.a * d, n = -1 * this.a * this.lat0) : (s = this.a * Math.sin(i) / Math.tan(t), n = this.a * (ce(t - this.lat0) + (1 - Math.cos(i)) / Math.tan(t)));
  else if (Math.abs(t) <= g)
    s = this.a * d, n = -1 * this.ml0;
  else {
    var o = et(this.a, this.e, Math.sin(t)) / Math.tan(t);
...
function Q2(e) {
  ...
  if (s = e.x - this.x0, n = e.y - this.y0, this.sphere)
    if (Math.abs(n + this.a * this.lat0) <= g)
      a = M(s / this.a + this.long0, this.over), t = 0;
    else {
      d = this.lat0 + n / this.a, o = s * s / this.a / this.a + d * d, c = d;
      var l;
      for (i = Ut; i; --i)
        if (l = Math.tan(c), r = -1 * (d * (c * l + 1) - c - 0.5 * (c * c + o) * l) / ((c - d) / l - 1), c += r, Math.abs(r) <= g) {
          t = c;
          break;
        }
      a = M(this.long0 + Math.asin(s * Math.tan(c) / this.a) / Math.sin(t), this.over);
    }
  else if (Math.abs(n + this.ml0) <= g)
    t = 0, a = M(this.long0 + s / this.a, this.over);
  else {
    d = (this.ml0 + n) / this.a, o = s * s / this.a / this.a + d * d, c = d;
    var f, h, u, m, b;
    for (i = Ut; i; --i)
      if (b = this.e * Math.sin(c), f = Math.sqrt(1 - b * b) * Math.tan(c), h = this.a * D(this.e0, this.e1, this.e2, this.e3, c), u = this.e0 - 2 * this.e1 * Math.cos(2 * c) + 4 * this.e2 * Math.cos(4 * c) - 6 * this.e3 * Math.cos(6 * c), m = h / this.a, r = (d * (f * m + 1) - m - 0.5 * f * (m * m + o)) / (this.es * Math.sin(2 * c) * (m * m + o - 2 * d * m) / (4 * f) + (d - m) * (f * u - 2 / Math.sin(2 * c)) - u), c -= r, Math.abs(r) <= g) {
        t = c;
        break;
      }
...
var Z2 = ["Polyconic", "American_Polyconic", "poly"], ei = {
  init: W2,
  forward: J2,
  inverse: Q2,
  names: Z2
...
function ti(e) {
  var a, t = e.x, s = e.y, n = s - this.lat0, i = t - this.long0, d = n / Ne * 1e-5, o = i, c = 1, r = 0;
  for (a = 1; a <= 10; a++)
...
function si(e) {
  ...
  for (var m = 0; m < this.iterations; m++) {
    ...
    for (a = 2; a <= 6; a++)
      p = b * h - w * u, v = w * h + b * u, b = p, w = v, y = y + a * (this.B_re[a] * b - this.B_im[a] * w), B = B + a * (this.B_im[a] * b + this.B_re[a] * w);
    var S = y * y + B * B;
    h = (C * y + k * B) / S, u = (k * y - C * B) / S;
  }
  var x = h, E = u, j = 1, N = 0;
  for (a = 1; a <= 9; a++)
    j = j * x, N = N + this.D[a] * j;
  var J = this.lat0 + N * Ne * 1e5, Q = this.long0 + E;
  return e.x = Q, e.y = J, e;
}
var ni = ["New_Zealand_Map_Grid", "nzmg"], ii = {
  init: ai,
  forward: ti,
  inverse: si,
  names: ni
...
var ri = ["Miller_Cylindrical", "mill"], hi = {
  init: oi,
  forward: di,
  inverse: ci,
  names: ri
...
function li() {
  this.long0 = this.long0 || 0, this.sphere ? (this.n = 1, this.m = 0, this.es = 0, this.C_y = Math.sqrt((this.m + 1) / this.n), this.C_x = this.C_y / (this.m + 1)) : this.en = Xa(this.es);
}
function mi(e) {
  var a, t, s = e.x, n = e.y;
  if (s = M(s - this.long0, this.over), this.sphere) {
    if (!this.m)
      ...
      for (var i = this.n * Math.sin(n), d = fi; d; --d) {
    ...
    t = this.a * Be(n, c, r, this.en), a = this.a * s * r / Math.sqrt(1 - this.es * c * c);
  }
  return e.x = a, e.y = t, e;
}
function ui(e) {
  var a, t, s, n;
  return e.x -= this.x0, s = e.x / this.a, e.y -= this.y0, a = e.y / this.a, this.sphere ? (a /= this.C_y, s = s / (this.C_x * (this.m + Math.cos(a))), this.m ? a = de((this.m * a + Math.sin(a)) / this.n) : this.n !== 1 && (a = de(Math.sin(a) / this.n)), s = M(s + this.long0, this.over), a = ce(a)) : (a = Ya(e.y / this.a, this.es, this.en), n = Math.abs(a), n < _ ? (n = Math.sin(a), t = this.long0 + e.x * Math.sqrt(1 - this.es * n * n) / (this.a * Math.cos(a)), s = M(t, this.over)) : n - g < _ && (s = this.long0)), e.x = s, e.y = a, e;
}
var bi = ["Sinusoidal", "sinu"], wi = {
  init: li,
  forward: mi,
  inverse: ui,
  names: bi
...
function pi(e) {
  ...
  n /= 2, Math.PI / 2 - Math.abs(t) < g && (s = 0);
...
function yi() {
  Math.abs(this.lat1 + this.lat2) < g || (this.lat2 = this.lat2 || this.lat1, this.temp = this.b / this.a, this.es = 1 - Math.pow(this.temp, 2), this.e = Math.sqrt(this.es), this.e0 = We(this.es), this.e1 = Je(this.es), this.e2 = Qe(this.es), this.e3 = Ze(this.es), this.sin_phi = Math.sin(this.lat1), this.cos_phi = Math.cos(this.lat1), this.ms1 = te(this.e, this.sin_phi, this.cos_phi), this.ml1 = D(this.e0, this.e1, this.e2, this.e3, this.lat1), Math.abs(this.lat1 - this.lat2) < g ? this.ns = this.sin_phi : (this.sin_phi = Math.sin(this.lat2), this.cos_phi = Math.cos(this.lat2), this.ms2 = te(this.e, this.sin_phi, this.cos_phi), this.ml2 = D(this.e0, this.e1, this.e2, this.e3, this.lat2), this.ns = (this.ms1 - this.ms2) / (this.ml2 - this.ml1)), this.g = this.ml1 + this.ms1 / this.ns, this.ml0 = D(this.e0, this.e1, this.e2, this.e3, this.lat0), this.rh = this.a * (this.g - this.ml0));
}
function Mi(e) {
  var a = e.x, t = e.y, s;
  if (this.sphere)
...
function Bi(e) {
  ...
  if (t !== 0 && (i = Math.atan2(a * e.x, a * e.y)), this.sphere)
    return n = M(this.long0 + i / this.ns, this.over), s = ce(this.g - t / this.a), e.x = n, e.y = s, e;
  var d = this.g - t / this.a;
  return s = za(d, this.e0, this.e1, this.e2, this.e3), n = M(this.long0 + i / this.ns, this.over), e.x = n, e.y = s, e;
}
var Ci = ["Equidistant_Conic", "eqdc"], ki = {
  init: yi,
  forward: Mi,
  inverse: Bi,
  names: Ci
...
function Ei(e) {
  ...
  Math.abs(t) <= g && (n = this.x0 + this.R * s, i = this.y0);
  var d = de(2 * Math.abs(t / Math.PI));
  (Math.abs(s) <= g || Math.abs(Math.abs(t) - _) <= g) && (n = this.x0, t >= 0 ? i = this.y0 + Math.PI * this.R * Math.tan(0.5 * d) : i = this.y0 + Math.PI * this.R * -Math.tan(0.5 * d));
...
function Pi(e) {
  ...
  return e.x -= this.x0, e.y -= this.y0, f = Math.PI * this.R, s = e.x / f, n = e.y / f, i = s * s + n * n, d = -Math.abs(n) * (1 + i), o = d - 2 * n * n + s * s, c = -2 * d + 1 + 2 * n * n + i * i, u = n * n / c + (2 * o * o * o / c / c / c - 9 * d * o / c / c) / 27, r = (d - o * o / 3 / c) / c, l = 2 * Math.sqrt(-r / 3), f = 3 * u / r / l, Math.abs(f) > 1 && (f >= 0 ? f = 1 : f = -1), h = Math.acos(f) / 3, e.y >= 0 ? t = (-l * Math.cos(h + Math.PI / 3) - o / 3 / c) * Math.PI : t = -(-l * Math.cos(h + Math.PI / 3) - o / 3 / c) * Math.PI, Math.abs(s) < g ? a = this.long0 : a = M(this.long0 + Math.PI * (i - 1 + Math.sqrt(1 + 2 * (s * s - n * n) + i * i)) / 2 / s, this.over), e.x = a, e.y = t, e;
}
var Ai = ["Van_der_Grinten_I", "VanDerGrinten", "Van_der_Grinten", "vandg"], xi = {
  init: Si,
  forward: Ei,
  inverse: Pi,
  names: Ai
};
function Gi(e, a, t, s, n, i) {
  const d = s - a, o = Math.atan((1 - i) * Math.tan(e)), c = Math.atan((1 - i) * Math.tan(t)), r = Math.sin(o), l = Math.cos(o), f = Math.sin(c), h = Math.cos(c);
  let u = d, m, b = 100, w, p, v, C, k, y, B, S, x, E, j, N, J, Q;
  do {
    if (w = Math.sin(u), p = Math.cos(u), v = Math.sqrt(
      h * w * (h * w) + (l * f - r * h * p) * (l * f - r * h * p)
    ), v === 0)
      return { azi1: 0, s12: 0 };
    C = r * f + l * h * p, k = Math.atan2(v, C), y = l * h * w / v, B = 1 - y * y, S = B !== 0 ? C - 2 * r * f / B : 0, x = i / 16 * B * (4 + i * (4 - 3 * B)), m = u, u = d + (1 - x) * i * y * (k + x * v * (S + x * C * (-1 + 2 * S * S)));
  } while (Math.abs(u - m) > 1e-12 && --b > 0);
  return b === 0 ? { azi1: NaN, s12: NaN } : (E = B * (n * n - n * (1 - i) * (n * (1 - i))) / (n * (1 - i) * (n * (1 - i))), j = 1 + E / 16384 * (4096 + E * (-768 + E * (320 - 175 * E))), N = E / 1024 * (256 + E * (-128 + E * (74 - 47 * E))), J = N * v * (S + N / 4 * (C * (-1 + 2 * S * S) - N / 6 * S * (-3 + 4 * v * v) * (-3 + 4 * S * S))), Q = n * (1 - i) * j * (k - J), { azi1: Math.atan2(h * w, l * f - r * h * p), s12: Q });
}
function Ui(e, a, t, s, n, i) {
  ...
  let p = s / (n * (1 - i) * b), v, C = 100, k, y, B, S;
  do
    k = Math.cos(2 * f + p), y = Math.sin(p), B = Math.cos(p), S = w * y * (k + w / 4 * (B * (-1 + 2 * k * k) - w / 6 * k * (-3 + 4 * y * y) * (-3 + 4 * k * k))), v = p, p = s / (n * (1 - i) * b) + S;
  while (Math.abs(p - v) > 1e-12 && --C > 0);
  if (C === 0)
    return { lat2: NaN, lon2: NaN };
  const x = o * y - c * B * l, E = Math.atan2(
    o * B + c * y * l,
    (1 - i) * Math.sqrt(h * h + x * x)
  ), j = Math.atan2(
    y * r,
    c * B - o * y * l
  ), N = i / 16 * u * (4 + i * (4 - 3 * u)), J = j - (1 - N) * i * h * (p + N * y * (k + N * B * (-1 + 2 * k * k))), Q = a + J;
  return { lat2: E, lon2: Q };
}
function Ii() {
...
function ji(e) {
  ...
  return this.sphere ? Math.abs(this.sin_p12 - 1) <= g ? (e.x = this.x0 + this.a * (_ - t) * Math.sin(i), e.y = this.y0 - this.a * (_ - t) * Math.cos(i), e) : Math.abs(this.sin_p12 + 1) <= g ? (e.x = this.x0 + this.a * (_ + t) * Math.sin(i), e.y = this.y0 + this.a * (_ + t) * Math.cos(i), e) : (m = this.sin_p12 * s + this.cos_p12 * n * Math.cos(i), h = Math.acos(m), u = h ? h / Math.sin(h) : 1, e.x = this.x0 + this.a * u * n * Math.sin(i), e.y = this.y0 + this.a * u * (this.cos_p12 * s - this.sin_p12 * n * Math.cos(i)), e) : (d = We(this.es), o = Je(this.es), c = Qe(this.es), r = Ze(this.es), Math.abs(this.sin_p12 - 1) <= g ? (l = this.a * D(d, o, c, r, _), f = this.a * D(d, o, c, r, t), e.x = this.x0 + (l - f) * Math.sin(i), e.y = this.y0 - (l - f) * Math.cos(i), e) : Math.abs(this.sin_p12 + 1) <= g ? (l = this.a * D(d, o, c, r, _), f = this.a * D(d, o, c, r, t), e.x = this.x0 + (l + f) * Math.sin(i), e.y = this.y0 + (l + f) * Math.cos(i), e) : Math.abs(a) < g && Math.abs(t - this.lat0) < g ? (e.x = e.y = 0, e) : (b = Gi(this.lat0, this.long0, t, a, this.a, this.f), w = b.azi1, e.x = b.s12 * Math.sin(w), e.y = b.s12 * Math.cos(w), e));
}
function Ni(e) {
  e.x -= this.x0, e.y -= this.y0;
  var a, t, s, n, i, d, o, c, r, l, f, h, u, m, b, w;
  return this.sphere ? (a = Math.sqrt(e.x * e.x + e.y * e.y), a > 2 * _ * this.a ? void 0 : (t = a / this.a, s = Math.sin(t), n = Math.cos(t), i = this.long0, Math.abs(a) <= g ? d = this.lat0 : (d = de(n * this.sin_p12 + e.y * s * this.cos_p12 / a), o = Math.abs(this.lat0) - _, Math.abs(o) <= g ? this.lat0 >= 0 ? i = M(this.long0 + Math.atan2(e.x, -e.y), this.over) : i = M(this.long0 - Math.atan2(-e.x, e.y), this.over) : i = M(this.long0 + Math.atan2(e.x * s, a * this.cos_p12 * n - e.y * this.sin_p12 * s), this.over)), e.x = i, e.y = d, e)) : (c = We(this.es), r = Je(this.es), l = Qe(this.es), f = Ze(this.es), Math.abs(this.sin_p12 - 1) <= g ? (h = this.a * D(c, r, l, f, _), a = Math.sqrt(e.x * e.x + e.y * e.y), u = h - a, d = za(u / this.a, c, r, l, f), i = M(this.long0 + Math.atan2(e.x, -1 * e.y), this.over), e.x = i, e.y = d, e) : Math.abs(this.sin_p12 + 1) <= g ? (h = this.a * D(c, r, l, f, _), a = Math.sqrt(e.x * e.x + e.y * e.y), u = a - h, d = za(u / this.a, c, r, l, f), i = M(this.long0 + Math.atan2(e.x, e.y), this.over), e.x = i, e.y = d, e) : (m = Math.atan2(e.x, e.y), b = Math.sqrt(e.x * e.x + e.y * e.y), w = Ui(this.lat0, this.long0, m, b, this.a, this.f), e.x = w.lon2, e.y = w.lat2, e));
}
var Ti = ["Azimuthal_Equidistant", "aeqd"], Ri = {
  init: Ii,
  forward: ji,
  inverse: Ni,
  names: Ti
...
function Di(e) {
  var a, t, s, n, i, d, o, c, r = e.x, l = e.y;
  return s = M(r - (this.long0 || 0), this.over), a = Math.sin(l), t = Math.cos(l), n = Math.cos(s), d = this.sin_p14 * a + this.cos_p14 * t * n, i = 1, (d > 0 || Math.abs(d) <= g) && (o = this.a * i * t * Math.sin(s), c = (this.y0 || 0) + this.a * i * (this.cos_p14 * a - this.sin_p14 * t * n)), e.x = o, e.y = c, e;
}
function Fi(e) {
  var a, t, s, n, i, d, o, c, r;
  return e.x -= this.x0 || 0, e.y -= this.y0 || 0, a = Math.sqrt(e.x * e.x + e.y * e.y), t = de(a / this.a), s = Math.sin(t), n = Math.cos(t), c = this.long0 || 0, r = this.lat0 || 0, d = c, Math.abs(a) <= g ? (o = r, e.x = d, e.y = o, e) : (o = de(n * this.sin_p14 + e.y * s * this.cos_p14 / a), i = Math.abs(r) - _, Math.abs(i) <= g ? (r >= 0 ? d = M(c + Math.atan2(e.x, -e.y), this.over) : d = M(c - Math.atan2(-e.x, e.y), this.over), e.x = d, e.y = o, e) : (d = M(c + Math.atan2(e.x * s, a * this.cos_p14 * n - e.y * this.sin_p14 * s), this.over), e.x = d, e.y = o, e));
}
var Li = ["ortho"], qi = {
  init: Oi,
  forward: Di,
...
function Vi() {
  this.x0 = this.x0 || 0, this.y0 = this.y0 || 0, this.lat0 = this.lat0 || 0, this.long0 = this.long0 || 0, this.lat_ts = this.lat_ts || 0, this.title = this.title || "Quadrilateralized Spherical Cube", this.lat0 >= _ - P / 2 ? this.face = G.TOP : this.lat0 <= -(_ - P / 2) ? this.face = G.BOTTOM : Math.abs(this.long0) <= P ? this.face = G.FRONT : Math.abs(this.long0) <= _ + P ? this.face = this.long0 > 0 ? G.RIGHT : G.LEFT : this.face = G.BACK, this.es !== 0 && (this.one_minus_f = 1 - (this.a - this.b) / this.a, this.one_minus_f_squared = this.one_minus_f * this.one_minus_f);
}
function Hi(e) {
  var a = { x: 0, y: 0 }, t, s, n, i, d, o, c = { value: 0 };
  if (e.x -= this.long0, this.es !== 0 ? t = Math.atan(this.one_minus_f_squared * Math.tan(e.y)) : t = e.y, s = e.x, this.face === G.TOP)
    i = _ - t, s >= P && s <= _ + P ? (c.value = A.AREA_0, n = s - _) : s > _ + P || s <= -(_ + P) ? (c.value = A.AREA_1, n = s > 0 ? s - I : s + I) : s > -(_ + P) && s <= -P ? (c.value = A.AREA_2, n = s + _) : (c.value = A.AREA_3, n = s);
  else if (this.face === G.BOTTOM)
    i = _ + t, s >= P && s <= _ + P ? (c.value = A.AREA_0, n = -s + _) : s < P && s >= -P ? (c.value = A.AREA_1, n = -s) : s < -P && s >= -(_ + P) ? (c.value = A.AREA_2, n = -s - _) : (c.value = A.AREA_3, n = s > 0 ? -s + I : -s - I);
  else {
    var r, l, f, h, u, m, b;
    this.face === G.RIGHT ? s = ge(s, +_) : this.face === G.BACK ? s = ge(s, +I) : this.face === G.LEFT && (s = ge(s, -_)), h = Math.sin(t), u = Math.cos(t), m = Math.sin(s), b = Math.cos(s), r = u * b, l = u * m, f = h, this.face === G.FRONT ? (i = Math.acos(r), n = ia(i, f, l, c)) : this.face === G.RIGHT ? (i = Math.acos(l), n = ia(i, f, -r, c)) : this.face === G.BACK ? (i = Math.acos(-r), n = ia(i, f, -l, c)) : this.face === G.LEFT ? (i = Math.acos(-l), n = ia(i, f, r, c)) : (i = n = 0, c.value = A.AREA_0);
  }
  return o = Math.atan(12 / I * (n + Math.acos(Math.sin(n) * Math.cos(P)) - _)), d = Math.sqrt((1 - Math.cos(i)) / (Math.cos(o) * Math.cos(o)) / (1 - Math.cos(Math.atan(1 / Math.cos(n))))), c.value === A.AREA_1 ? o += _ : c.value === A.AREA_2 ? o += I : c.value === A.AREA_3 && (o += 1.5 * I), a.x = d * Math.cos(o), a.y = d * Math.sin(o), a.x = a.x * this.a + this.x0, a.y = a.y * this.a + this.y0, e.x = a.x, e.y = a.y, e;
}
function $i(e) {
  var a = { lam: 0, phi: 0 }, t, s, n, i, d, o, c, r, l, f = { value: 0 };
  if (e.x = (e.x - this.x0) / this.a, e.y = (e.y - this.y0) / this.a, s = Math.atan(Math.sqrt(e.x * e.x + e.y * e.y)), t = Math.atan2(e.y, e.x), e.x >= 0 && e.x >= Math.abs(e.y) ? f.value = A.AREA_0 : e.y >= 0 && e.y >= Math.abs(e.x) ? (f.value = A.AREA_1, t -= _) : e.x < 0 && -e.x >= Math.abs(e.y) ? (f.value = A.AREA_2, t = t < 0 ? t + I : t - I) : (f.value = A.AREA_3, t += _), l = I / 12 * Math.tan(t), d = Math.sin(l) / (Math.cos(l) - 1 / Math.sqrt(2)), o = Math.atan(d), n = Math.cos(t), i = Math.tan(s), c = 1 - n * n * i * i * (1 - Math.cos(Math.atan(1 / Math.cos(o)))), c < -1 ? c = -1 : c > 1 && (c = 1), this.face === G.TOP)
    r = Math.acos(c), a.phi = _ - r, f.value === A.AREA_0 ? a.lam = o + _ : f.value === A.AREA_1 ? a.lam = o < 0 ? o + I : o - I : f.value === A.AREA_2 ? a.lam = o - _ : a.lam = o;
  else if (this.face === G.BOTTOM)
    r = Math.acos(c), a.phi = r - _, f.value === A.AREA_0 ? a.lam = -o + _ : f.value === A.AREA_1 ? a.lam = -o : f.value === A.AREA_2 ? a.lam = -o - _ : a.lam = o < 0 ? -o - I : -o + I;
  else {
    var h, u, m;
    h = c, l = h * h, l >= 1 ? m = 0 : m = Math.sqrt(1 - l) * Math.sin(o), l += m * m, l >= 1 ? u = 0 : u = Math.sqrt(1 - l), f.value === A.AREA_1 ? (l = u, u = -m, m = l) : f.value === A.AREA_2 ? (u = -u, m = -m) : f.value === A.AREA_3 && (l = u, u = m, m = -l), this.face === G.RIGHT ? (l = h, h = -u, u = l) : this.face === G.BACK ? (h = -h, u = -u) : this.face === G.LEFT && (l = h, h = u, u = -l), a.phi = Math.acos(-m) - _, a.lam = Math.atan2(u, h), this.face === G.RIGHT ? a.lam = ge(a.lam, -_) : this.face === G.BACK ? a.lam = ge(a.lam, -I) : this.face === G.LEFT && (a.lam = ge(a.lam, +_));
  }
  if (this.es !== 0) {
    var b, w, p;
...
function ia(e, a, t, s) {
  var n;
  return e < g ? (s.value = A.AREA_0, n = 0) : (n = Math.atan2(a, t), Math.abs(n) <= P ? s.value = A.AREA_0 : n > P && n <= _ + P ? (s.value = A.AREA_1, n -= _) : n > _ + P || n <= -(_ + P) ? (s.value = A.AREA_2, n = n >= 0 ? n - I : n + I) : (s.value = A.AREA_3, n += _)), n;
}
function ge(e, a) {
  var t = e + a;
  return t < -I ? t += Fe : t > +I && (t -= Fe), t;
}
var Ki = ["Quadrilateralized Spherical Cube", "Quadrilateralized_Spherical_Cube", "qsc"], Xi = {
  init: Vi,
  forward: Hi,
  inverse: $i,
  names: Ki
...
], cs = 0.8487, rs = 1.3523, hs = V / 5, Yi = 1 / hs, ze = 18, pa = function(e, a) {
...
function Zi(e) {
  var a = M(e.x - this.long0, this.over), t = Math.abs(e.y), s = Math.floor(t * hs);
  s < 0 ? s = 0 : s >= ze && (s = ze - 1), t = V * (t - Yi * s);
  var n = {
    x: pa(Fa[s], t) * a,
    y: pa(Ie[s], t)
  };
  return e.y < 0 && (n.y = -n.y), n.x = n.x * this.a * cs + this.x0, n.y = n.y * this.a * rs + this.y0, n;
}
function e0(e) {
  var a = {
    x: (e.x - this.x0) / (this.a * cs),
    y: Math.abs(e.y - this.y0) / (this.a * rs)
  };
  if (a.y >= 1)
    a.x /= Fa[ze][0], a.y = e.y < 0 ? -_ : _;
  else {
    var t = Math.floor(a.y * ze);
    for (t < 0 ? t = 0 : t >= ze && (t = ze - 1); ; )
      if (Ie[t][0] > a.y)
        --t;
      else if (Ie[t + 1][0] <= a.y)
        ++t;
      else
        break;
    var s = Ie[t], n = 5 * (a.y - s[0]) / (Ie[t + 1][0] - s[0]);
    n = Ji(function(i) {
      return (pa(s, i) - a.y) / Wi(s, i);
    }, n, g, 100), a.x /= pa(Fa[t], n), a.y = (5 * t + n) * U, e.y < 0 && (a.y = -a.y);
  }
  return a.x = M(a.x + this.long0, this.over), a;
}
var a0 = ["Robinson", "robin"], t0 = {
  init: Qi,
  forward: Zi,
  inverse: e0,
  names: a0
...
function n0(e) {
  var a = Jt(e, this.es, this.a);
  return a;
}
function i0(e) {
  var a = Qt(e, this.es, this.a, this.b);
  return a;
}
var o0 = ["Geocentric", "geocentric", "geocent", "Geocent"], d0 = {
  init: s0,
  forward: n0,
  inverse: i0,
  names: o0
...
function c0() {
  if (Object.keys(xe).forEach(function(t) {
    if (typeof this[t] > "u")
      this[t] = xe[t].def;
    else {
      if (xe[t].num && isNaN(this[t]))
        throw new Error("Invalid parameter value, must be numeric " + t + " = " + this[t]);
      xe[t].num && (this[t] = parseFloat(this[t]));
    }
    xe[t].degrees && (this[t] = this[t] * U);
  }.bind(this)), Math.abs(Math.abs(this.lat0) - _) < g ? this.mode = this.lat0 < 0 ? O.S_POLE : O.N_POLE : Math.abs(this.lat0) < g ? this.mode = O.EQUIT : (this.mode = O.OBLIQ, this.sinph0 = Math.sin(this.lat0), this.cosph0 = Math.cos(this.lat0)), this.pn1 = this.h / this.a, this.pn1 <= 0 || this.pn1 > 1e10)
...
var f0 = ["Tilted_Perspective", "tpers"], l0 = {
  init: c0,
  forward: r0,
  inverse: h0,
  names: f0
...
function u0(e) {
  ...
  if (a = a - this.long0, this.shape === "ellipse") {
    ...
    var o = this.radius_p / q(this.radius_p * Math.cos(t), Math.sin(t));
    ...
    s = this.radius_g - n, this.flip_axis ? (e.x = this.radius_g_1 * Math.atan(i / q(d, s)), e.y = this.radius_g_1 * Math.atan(d / s)) : (e.x = this.radius_g_1 * Math.atan(i / s), e.y = this.radius_g_1 * Math.atan(d / q(i, s)));
  } else this.shape === "sphere" && (s = Math.cos(t), n = Math.cos(a) * s, i = Math.sin(a) * s, d = Math.sin(t), s = this.radius_g - n, this.flip_axis ? (e.x = this.radius_g_1 * Math.atan(i / q(d, s)), e.y = this.radius_g_1 * Math.atan(d / s)) : (e.x = this.radius_g_1 * Math.atan(i / s), e.y = this.radius_g_1 * Math.atan(d / q(i, s))));
  return e.x = e.x * this.a, e.y = e.y * this.a, e;
}
function b0(e) {
  var a = -1, t = 0, s = 0, n, i, d, o;
  if (e.x = e.x / this.a, e.y = e.y / this.a, this.shape === "ellipse") {
    this.flip_axis ? (s = Math.tan(e.y / this.radius_g_1), t = Math.tan(e.x / this.radius_g_1) * q(1, s)) : (t = Math.tan(e.x / this.radius_g_1), s = Math.tan(e.y / this.radius_g_1) * q(1, t));
...
var w0 = ["Geostationary Satellite View", "Geostationary_Satellite", "geos"], z0 = {
  init: m0,
  forward: u0,
  inverse: b0,
  names: w0
}, Te = 1.340264, Re = -0.081106, Oe = 893e-6, De = 3796e-6, _a = Math.sqrt(3) / 2;
function p0() {
  this.es = 0, this.long0 = this.long0 !== void 0 ? this.long0 : 0, this.x0 = this.x0 !== void 0 ? this.x0 : 0, this.y0 = this.y0 !== void 0 ? this.y0 : 0;
}
function _0(e) {
  var a = M(e.x - this.long0, this.over), t = e.y, s = Math.asin(_a * Math.sin(t)), n = s * s, i = n * n * n;
  return e.x = a * Math.cos(s) / (_a * (Te + 3 * Re * n + i * (7 * Oe + 9 * De * n))), e.y = s * (Te + Re * n + i * (Oe + De * n)), e.x = this.a * e.x + this.x0, e.y = this.a * e.y + this.y0, e;
}
function v0(e) {
  e.x = (e.x - this.x0) / this.a, e.y = (e.y - this.y0) / this.a;
  var a = 1e-9, t = 12, s = e.y, n, i, d, o, c, r;
  for (r = 0; r < t && (n = s * s, i = n * n * n, d = s * (Te + Re * n + i * (Oe + De * n)) - e.y, o = Te + 3 * Re * n + i * (7 * Oe + 9 * De * n), s -= c = d / o, !(Math.abs(c) < a)); ++r)
    ;
  return n = s * s, i = n * n * n, e.x = _a * e.x * (Te + 3 * Re * n + i * (7 * Oe + 9 * De * n)) / Math.cos(s), e.y = Math.asin(Math.sin(s) / _a), e.x = M(e.x + this.long0, this.over), e;
}
var g0 = ["eqearth", "Equal Earth", "Equal_Earth"], y0 = {
  init: p0,
  forward: _0,
  inverse: v0,
  names: g0
...
function M0() {
  ...
  if (this.phi1 = this.lat1, Math.abs(this.phi1) < Ve)
    throw new Error();
  this.es ? (this.en = Xa(this.es), this.m1 = Be(
    this.phi1,
    this.am1 = Math.sin(this.phi1),
    e = Math.cos(this.phi1),
    this.en
  ), this.am1 = e / (Math.sqrt(1 - this.es * this.am1 * this.am1) * this.am1), this.inverse = C0, this.forward = B0) : (Math.abs(this.phi1) + Ve >= _ ? this.cphi1 = 0 : this.cphi1 = 1 / Math.tan(this.phi1), this.inverse = S0, this.forward = k0);
}
function B0(e) {
  var a = M(e.x - (this.long0 || 0), this.over), t = e.y, s, n, i;
  return s = this.am1 + this.m1 - Be(t, n = Math.sin(t), i = Math.cos(t), this.en), n = i * a / (s * Math.sqrt(1 - this.es * n * n)), e.x = s * Math.sin(n), e.y = this.am1 - s * Math.cos(n), e.x = this.a * e.x + (this.x0 || 0), e.y = this.a * e.y + (this.y0 || 0), e;
}
function C0(e) {
  e.x = (e.x - (this.x0 || 0)) / this.a, e.y = (e.y - (this.y0 || 0)) / this.a;
  var a, t, s, n;
  if (t = q(e.x, e.y = this.am1 - e.y), n = Ya(this.am1 + this.m1 - t, this.es, this.en), (a = Math.abs(n)) < _)
    a = Math.sin(n), s = t * Math.atan2(e.x, e.y) * Math.sqrt(1 - this.es * a * a) / Math.cos(n);
  else if (Math.abs(a - _) <= Ve)
    s = 0;
  else
    throw new Error();
  return e.x = M(s + (this.long0 || 0), this.over), e.y = ce(n), e;
}
function k0(e) {
  var a = M(e.x - (this.long0 || 0), this.over), t = e.y, s, n;
  return n = this.cphi1 + this.phi1 - t, Math.abs(n) > Ve ? (e.x = n * Math.sin(s = a * Math.cos(t) / n), e.y = this.cphi1 - n * Math.cos(s)) : e.x = e.y = 0, e.x = this.a * e.x + (this.x0 || 0), e.y = this.a * e.y + (this.y0 || 0), e;
}
function S0(e) {
  e.x = (e.x - (this.x0 || 0)) / this.a, e.y = (e.y - (this.y0 || 0)) / this.a;
  var a, t, s = q(e.x, e.y = this.cphi1 - e.y);
  if (t = this.cphi1 + this.phi1 - s, Math.abs(t) > _)
    throw new Error();
  return Math.abs(Math.abs(t) - _) <= Ve ? a = 0 : a = s * Math.atan2(e.x, e.y) / Math.cos(t), e.x = M(a + (this.long0 || 0), this.over), e.y = ce(t), e;
}
var E0 = ["bonne", "Bonne (Werner lat_1=90)"], P0 = {
  init: M0,
  names: E0
};
const It = {
...
function A0() {
  if (this.x0 = this.x0 || 0, this.y0 = this.y0 || 0, this.long0 = this.long0 || 0, this.title = this.title || "General Oblique Transformation", this.isIdentity = Kt.includes(this.o_proj), !this.o_proj)
    throw new Error("Missing parameter: o_proj");
  if (this.o_proj === "ob_tran")
    throw new Error("Invalid value for o_proj: " + this.o_proj);
  const e = this.projStr.replace("+proj=ob_tran", "").replace("+o_proj=", "+proj=").trim(), a = W(e);
  if (!a)
    throw new Error("Invalid parameter: o_proj. Unknown projection " + this.o_proj);
  a.long0 = 0, this.obliqueProjection = a;
  let t;
  const s = Object.keys(va), n = (o) => {
    if (typeof this[o] > "u")
      return;
  ...
  for (let o = 0; o < s.length; o++) {
    const c = s[o], r = va[c], l = Object.entries(r);
    if (l.some(
      ([h]) => typeof this[h] < "u"
    )) {
      t = r;
  ...
  const { lamp: i, phip: d } = U0(this, t);
  this.lamp = i, Math.abs(d) > g ? (this.cphip = Math.cos(d), this.sphip = Math.sin(d), this.projectionType = It.OBLIQUE) : this.projectionType = It.TRANSVERSE;
}
function x0(e) {
...
function I0(e, a) {
  ...
  return e.isIdentity && (o.x *= V, o.y *= V), o;
}
function j0(e, a) {
  let { x: t, y: s } = a;
  t += e.long0;
  ...
  return e.isIdentity && (d.x *= V, d.y *= V), d;
}
function N0(e, a) {
...
var R0 = ["General Oblique Transformation", "General_Oblique_Transformation", "ob_tran"], O0 = {
  init: A0,
  forward: x0,
  inverse: G0,
  names: R0
...
function D0(e) {
  e.Proj.projections.add(ra), e.Proj.projections.add(ha), e.Proj.projections.add(E1), e.Proj.projections.add(T1), e.Proj.projections.add(L1), e.Proj.projections.add(K1), e.Proj.projections.add(Z1), e.Proj.projections.add(n2), e.Proj.projections.add(r2), e.Proj.projections.add(u2), e.Proj.projections.add(S2), e.Proj.projections.add(U2), e.Proj.projections.add(R2), e.Proj.projections.add(V2), e.Proj.projections.add(Y2), e.Proj.projections.add(ei), e.Proj.projections.add(ii), e.Proj.projections.add(hi), e.Proj.projections.add(wi), e.Proj.projections.add(gi), e.Proj.projections.add(ki), e.Proj.projections.add(xi), e.Proj.projections.add(Ri), e.Proj.projections.add(qi), e.Proj.projections.add(Xi), e.Proj.projections.add(t0), e.Proj.projections.add(d0), e.Proj.projections.add(l0), e.Proj.projections.add(z0), e.Proj.projections.add(y0), e.Proj.projections.add(P0), e.Proj.projections.add(O0);
}
const at = Object.assign(Hn, {
  defaultDatum: "WGS84",
  Proj: W,
  WGS84: new W("WGS84"),
  Point: Me,
...
D0(at);
const fs = "+proj=tmerc +lat_0=0 +lon_0=173 +k=0.9996 +x_0=1600000 +y_0=10000000 +ellps=GRS80 +units=m +no_defs", K = 512, ga = 1e7, F0 = (e) => at("EPSG:4326", fs, e), L0 = (e) => at(fs, "EPSG:4326", e), q0 = (e, a, t = K) => [[e, a], [e + t, a], [e + t, a - t], [e, a - t]].map(L0);
function V0(e, a) {
  let t = !1;
...
function* $0(e) {
  ...
  for (const t of e) {
    ...
    for (let i = Math.floor(Math.min(...s) / K); i <= Math.floor(Math.max(...s) / K); i++)
      for (let d = Math.floor((ga - Math.max(...n)) / K); d <= Math.floor((ga - Math.min(...n)) / K); d++) {
        const o = `${i}:${d}`;
...
function K0(e, a = K) {
  const t = a + 2, s = new Uint8Array(Math.ceil(a * a / 4));
  for (let n = 0; n < a; n++) for (let i = 0; i < a; i++) {
    const d = n * t + i, o = [e[d], e[d + 1], e[d + 2], e[d + t], e[d + t + 1], e[d + t + 2], e[d + 2 * t], e[d + 2 * t + 1], e[d + 2 * t + 2]];
    if (!o.every(Number.isFinite)) continue;
    const c = (o[2] + 2 * o[5] + o[8] - o[0] - 2 * o[3] - o[6]) / 8, r = (o[6] + 2 * o[7] + o[8] - o[0] - 2 * o[1] - o[2]) / 8, l = Math.atan(Math.hypot(c, r)) * 180 / Math.PI > 35 ? 2 : 1, f = n * a + i;
    s[f >> 2] |= l << f % 4 * 2;
  }
  return s;
}
function X0(e, a, t, s, n = K) {
  const i = e.slice();
  for (let d = 0; d < n; d++) for (let o = 0; o < n; o++) if (!s.some((c) => V0([a + o + 0.5, t - d - 0.5], c))) {
    const c = d * n + o;
    i[c >> 2] &= ~(3 << c % 4 * 2);
  }
  return i;
...
const {
  ...
  create: st,
  defineProperty: Z0
} = Q0, eo = Array, ao = eo.prototype, ms = ao[ea], to = R(ms), us = ArrayBuffer, so = us.prototype;
Ce(so, "byteLength");
const jt = typeof SharedArrayBuffer < "u" ? SharedArrayBuffer : null;
jt && Ce(jt.prototype, "byteLength");
const bs = tt(Uint8Array);
bs.from;
const F = bs.prototype;
F[ea];
R(F.keys);
R(
  F.values
...
const no = Uint8Array, ws = Uint16Array, nt = Uint32Array, io = Float32Array, He = tt([][ea]()), zs = R(He.next), oo = R((function* () {
})().next), co = tt(He), ro = DataView.prototype, ho = R(
  ro.getUint16
), it = WeakMap, ps = it.prototype, _s = R(ps.get), fo = R(ps.set), vs = new it(), lo = st(null, {
  next: {
    value: function() {
      const a = _s(vs, this);
      return zs(a);
...
function mo(e) {
  if (e[ea] === ms && He.next === zs)
    return e;
  const a = st(lo);
  return fo(vs, a, to(e)), a;
}
const uo = new it(), bo = st(co, {
  next: {
    value: function() {
      const a = _s(uo, this);
      return oo(a);
...
for (const e of W0(He))
  e !== "next" && Z0(bo, e, ls(He, e));
const gs = new us(4), wo = new io(gs), zo = new nt(gs), ee = new ws(512), ae = new no(512);
for (let e = 0; e < 256; ++e) {
  const a = e - 127;
  a < -24 ? (ee[e] = 0, ee[e | 256] = 32768, ae[e] = 24, ae[e | 256] = 24) : a < -14 ? (ee[e] = 1024 >> -a - 14, ee[e | 256] = 1024 >> -a - 14 | 32768, ae[e] = -a - 1, ae[e | 256] = -a - 1) : a <= 15 ? (ee[e] = a + 15 << 10, ee[e | 256] = a + 15 << 10 | 32768, ae[e] = 13, ae[e | 256] = 13) : a < 128 ? (ee[e] = 31744, ee[e | 256] = 64512, ae[e] = 24, ae[e | 256] = 24) : (ee[e] = 31744, ee[e | 256] = 64512, ae[e] = 13, ae[e | 256] = 13);
}
const ot = new nt(2048);
for (let e = 1; e < 1024; ++e) {
  let a = e << 13, t = 0;
  for (; (a & 8388608) === 0; )
    a <<= 1, t -= 8388608;
  a &= -8388609, t += 947912704, ot[e] = a | t;
}
for (let e = 1024; e < 2048; ++e)
  ot[e] = 939524096 + (e - 1024 << 13);
const ke = new nt(64);
for (let e = 1; e < 31; ++e)
  ke[e] = e << 23;
ke[31] = 1199570944;
...
function po(e) {
  const a = e >> 10;
  return zo[0] = ot[ys[a] + (e & 1023)] + ke[a], wo[0];
}
function Ms(e, a, ...t) {
  return po(
    ho(e, a, ...mo(t))
...
function go(e, a, t) {
  const s = [];
  let i = 0, d;
  for (; d = vo(e, a, { debug: !1, startIndex: i }); )
...
function Bo(e, a, t, s = !1, n = !1) {
  Bs[a] = e, Ke[e] = { tag: e, name: a, type: typeof t == "string" ? z[t] : t, isArray: s, eager: n };
}
for (const [e, a] of Object.entries(Mo)) {
  const t = (
    /** @type {TagDictionaryEntry} */
    a
  );
  Bo(t.tag, t.name || e, t.type, t.isArray, t.eager);
}
function oa(e) {
  return typeof e == "number" ? e : Bs[e];
}
const $ = {
  WhiteIsZero: 0,
  BlackIsZero: 1,
...
for (const [e, a] of Object.entries(Cs))
  ;
function ko(e, a) {
...
function jo(e, a, t = la, s = !0) {
  Array.isArray(e) || (e = [e]), e.forEach((n) => {
    Xe.set(n, { importFn: a, decoderParameterFn: t, preferWorker: s });
  });
}
...
function Do(e, a, t, s, n) {
  ...
  return e.map((o) => {
    ...
    for (let r = 0; r < n; ++r) {
      ...
      for (let u = 0; u < s; ++u) {
        const m = i * u, b = m % 1, w = Math.floor(m), p = Math.min(Math.ceil(m), a - 1), v = o[f * a + w], C = o[f * a + p], k = o[h * a + w], y = o[h * a + p], B = ye(ye(v, C, b), ye(k, y, b), l % 1);
        c[r * s + u] = B;
      }
    }
    return c;
...
function Fo(e, a, t, s, n, i = "nearest") {
...
function Lo(e, a, t, s, n, i) {
  const d = a / s, o = t / n, c = Ba(e, s, n, i);
  for (let r = 0; r < n; ++r) {
...
function qo(e, a, t, s, n, i) {
  ...
  for (let r = 0; r < n; ++r) {
    ...
    for (let u = 0; u < s; ++u) {
      ...
      for (let v = 0; v < i; ++v) {
        const C = e[f * a * i + w * i + v], k = e[f * a * i + p * i + v], y = e[h * a * i + w * i + v], B = e[h * a * i + p * i + v], S = ye(ye(C, k, b), ye(y, B, b), l % 1);
        c[r * s * i + u * i + v] = S;
      }
    }
  }
...
function Vo(e, a, t, s, n, i, d = "nearest") {
  switch (d.toLowerCase()) {
    case "nearest":
      return Lo(e, a, t, s, n, i);
    case "bilinear":
    case "linear":
      return qo(e, a, t, s, n, i);
    default:
...
function Ko(e, a, t, s, n, i, d) {
  const o = new DataView(e), c = t === 2 ? d * i : d * i * s, r = t === 2 ? 1 : s, l = La(a, n, c), f = parseInt("1".repeat(n), 2);
  if (a === 1) {
    let h;
    t === 1 ? h = s * n : h = n;
    let u = i * h;
    ...
    for (let m = 0; m < d; ++m) {
      ...
      for (let w = 0; w < i; ++w) {
        const p = b + w * r * n;
        for (let v = 0; v < r; ++v) {
          const C = p + v * n, k = (m * i + w) * r + v, y = Math.floor(C / 8), B = C % 8;
          if (B + n <= 8)
            l[k] = o.getUint8(y) >> 8 - n - B & f;
          else if (B + n <= 16)
            l[k] = o.getUint16(y) >> 16 - n - B & f;
          else if (B + n <= 24) {
            const S = o.getUint16(y) << 8 | o.getUint8(y + 2);
            l[k] = S >> 24 - n - B & f;
          } else
            l[k] = o.getUint32(y) >> 32 - n - B & f;
        }
      }
    }
  }
...
class Xo {
  ...
  async getTileOrStrip(a, t, s, n, i) {
    ...
    return r === null || !r[c] ? (u = (async () => {
      ...
      return $o(b, w) && (m = Ko(m, b, this.planarConfiguration, this.getSamplesPerPixel(), w, this.getTileWidth(), this.getBlockHeight(t))), m;
  ...
  async _readRaster(a, t, s, n, i, d, o, c, r) {
    ...
    for (let E = 0; E < t.length; ++E) {
      if (this.planarConfiguration === 1) {
        const j = await this.fileDirectory.loadValue("BitsPerSample");
        if (typeof j != "object")
          throw new Error("Expected BitsPerSample to be an array or typed array.");
        k.push(Ho(j, 0, t[E]) / 8);
      } else
        k.push(0);
      y.push(this.getReaderForSample(t[E]));
    }
    const B = [], { littleEndian: S } = this;
    for (let E = w; E < p; ++E)
      for (let j = m; j < b; ++j) {
        let N;
        this.planarConfiguration === 1 && (N = this.getTileOrStrip(j, E, 0, i, r));
        for (let J = 0; J < t.length; ++J) {
          const Q = J, Ca = t[J];
          if (this.planarConfiguration === 2 && (C = this.getSampleByteSize(Ca), N = this.getTileOrStrip(j, E, Ca, i, r)), !N)
            throw new Error("Could not get tile or strip data.");
          const Es = N.then((Se) => {
            const Ps = Se.data, As = new DataView(Ps), ka = this.getBlockHeight(Se.y), Ee = Se.y * f, aa = Se.x * l, xs = Ee + ka, Gs = (Se.x + 1) * l, Us = y[Q], Is = Math.min(ka, ka - (xs - a[3]), u - Ee), js = Math.min(l, l - (Gs - a[2]), h - aa);
            for (let Pe = Math.max(0, a[1] - Ee); Pe < Is; ++Pe)
              for (let Ae = Math.max(0, a[0] - aa); Ae < js; ++Ae) {
                const Ns = (Pe * l + Ae) * C, lt = Us.call(As, Ns + k[Q], S);
                let ta;
                n ? (ta = (Pe + Ee - a[1]) * v * t.length + (Ae + aa - a[0]) * t.length + Q, s[ta] = lt) : (ta = (Pe + Ee - a[1]) * v + Ae + aa - a[0], s[Q][ta] = lt);
              }
          });
          B.push(Es);
        }
    ...
    if (await Promise.all(B), d && a[2] - a[0] !== d || o && a[3] - a[1] !== o) {
      ...
      return j.width = d ?? a[2] - a[0], j.height = o ?? a[3] - a[1], j;
    }
    const x = (
      /** @type {ReadRasterResult} */
      s
    );
    return x.width = d || a[2] - a[0], x.height = o || a[3] - a[1], x;
  }
  /**
   * @overload
  ...
  async readRasters(a = {}) {
    ...
    if (!s || !s.length)
      ...
      for (let y = 0; y < s.length; ++y)
        if (s[y] >= b)
          return Promise.reject(new RangeError(`Invalid sample index '${s[y]}'.`));
    let w;
    if (l) {
      const { fileDirectory: y } = this, B = y.getValue("SampleFormat"), S = B ? Math.max.apply(null, Array.from(B)) : 1;
      if (S !== 1 && S !== 2 && S !== 3)
        throw new Error("Unsupported sample format for interleaved data. Must be 1, 2, or 3.");
      const x = y.getValue("BitsPerSample"), E = x ? Math.max.apply(null, Array.from(x)) : 8;
      if (w = La(S, E, m * s.length), c) {
        if (Array.isArray(c))
      ...
      w = [];
      for (let y = 0; y < s.length; ++y) {
        const B = this.getArrayForSample(s[y], m);
        Array.isArray(c) && y < c.length ? B.fill(c[y]) : c && !Array.isArray(c) && B.fill(c), w.push(B);
      }
    }
    const p = this.fileDirectory.getValue("Compression") || 1, v = await No(p, this.fileDirectory), C = n ? n.bindParameters(p, v) : await To(p, v);
  ...
  async readRGB(a = {}) {
    ...
    if (f === $.RGB) {
      ...
      if (k && k[0] !== Co.Unspecified && o) {
        C = [];
        const y = this.fileDirectory.getValue("BitsPerSample") || [];
    ...
    if (!r) {
      const C = new Uint8Array(p.length / 3), k = new Uint8Array(p.length / 3), y = new Uint8Array(p.length / 3);
      for (let B = 0, S = 0; B < p.length; B += 3, ++S)
        C[S] = p[B], k[S] = p[B + 1], y[S] = p[B + 2];
      p = [C, k, y];
    }
    const v = (
...
function Qo(e, a) {
  ...
  for (; t < e.byteLength; ) {
    ...
    if (c.length === 0)
      break;
    const r = c.indexOf(Tt), l = Wo(c.substr(0, r)), { start: f, end: h, total: u } = qa(l["content-range"]), m = t + i.length + r + Tt.length, b = h + 1 - f;
    n.push({
      headers: l,
      data: e.slice(m, m + b),
      offset: f,
...
class le extends Error {
  constructor(...a) {
    super(...a), Error.captureStackTrace && Error.captureStackTrace(this, le), this.name = "AbortError", this.signal = void 0;
  }
}
...
class i3 extends Ss {
  ...
  constructor(a, { blockSize: t = 65536, cacheSize: s = 100 } = {}) {
    super(), this.source = a, this.blockSize = t, this.blockCache = new Zo({
      maxSize: s,
      onEviction: (n, i) => {
  ...
  async fetch(a, t) {
    ...
    if (l.length)
      throw new s3(l, "Request failed");
    const f = new Map(a3(i, r));
      ...
      for (let n = 0; n < t.length; ++n) {
        ...
        for (const d of i.blockIds)
          this.blockRequests.set(d, (async () => {
            try {
              const o = (await Promise.all(s))[n], c = d * this.blockSize, r = c - o.offset, l = Math.min(r + this.blockSize, o.data.byteLength), f = o.data.slice(r, l), h = new n3(
                c,
                f.byteLength,
                /** @type {ArrayBuffer} */
                f
              ...
              if (o instanceof le && o.name === "AbortError")
  ...
  groupBlocks(a) {
    ...
    const i = [];
    for (const d of t)
      n === null || n + 1 === d ? (s.push(d), n = d) : (i.push(new Rt(s[0] * this.blockSize, s.length * this.blockSize, s)), s = [d], n = d);
    return i.push(new Rt(s[0] * this.blockSize, s.length * this.blockSize, s)), i;
  }
  /**
   * @param {import("./basesource.js").Slice[]} slices
  ...
  constructRequest(a, t) {
    return new Promise((s, n) => {
      ...
      i.onload = () => {
        const d = i.response;
        s(new c3(i, d));
      }, i.onerror = n, i.onabort = () => n(new le("Request aborted")), i.send(), t && (t.aborted && i.abort(), t.addEventListener("abort", () => i.abort()));
    });
  }
...
class f3 extends ct {
  /** @param {string} url */
  constructor(a) {
    super(a), this.parsedUrl = Ua.parse(this.url), this.httpApi = (this.parsedUrl.protocol === "http:", Ua);
  }
  /**
   * @param {Object<string, string>} headers
   * @param {AbortSignal} [signal]
  ...
  constructRequest(a, t) {
    return new Promise((s, n) => {
      ...
      i.on("error", n), t && (t.aborted && i.destroy(new le("Request aborted")), t.addEventListener("abort", () => i.destroy(new le("Request aborted"))));
    });
  }
  async request({ headers: a = {}, signal: t = void 0 } = {}) {
    return await this.constructRequest(a, t);
...
class rt extends Ss {
  ...
  async fetchSlices(a, t) {
    ...
    if (s.ok)
      if (s.status === 206) {
        const { type: n, params: i } = Jo(s.getHeader("content-type"));
        if (n === "multipart/byteranges") {
          const f = Qo(await s.getData(), i.boundary);
          return this._fileSize = f[0].fileSize || null, f;
        }
        const d = await s.getData(), { start: o, end: c, total: r } = qa(s.getHeader("content-range"));
  ...
  async fetchSlice(a, t) {
    ...
    if (i.ok)
      if (i.status === 206) {
        const d = await i.getData(), { total: o } = qa(i.getHeader("content-range"));
...
function ht(e, { blockSize: a, cacheSize: t }) {
  return a === void 0 ? e : new i3(e, { blockSize: a, cacheSize: t });
}
function l3(e, { headers: a = {}, credentials: t, maxRanges: s = 0, allowFullFile: n = !1, ...i } = {}) {
  const d = new d3(e, t), o = new rt(d, { headers: a, maxRanges: s, allowFullFile: n });
  return ht(o, i);
}
function m3(e, { headers: a = {}, maxRanges: t = 0, allowFullFile: s = !1, ...n } = {}) {
  const i = new r3(e), d = new rt(i, { headers: a, maxRanges: t, allowFullFile: s });
  return ht(d, n);
}
function u3(e, { headers: a = {}, maxRanges: t = 0, allowFullFile: s = !1, ...n } = {}) {
  const i = new f3(e), d = new rt(i, { headers: a, maxRanges: t, allowFullFile: s });
  return ht(d, n);
}
function b3(e, { forceXHR: a = !1, ...t } = {}) {
  return typeof fetch == "function" && !a ? l3(e, t) : typeof XMLHttpRequest < "u" ? m3(e, t) : u3(e, t);
}
function pe(e, a) {
  switch (e) {
    case z.BYTE:
...
function je(e = null, a, t, s, n, i, d = !1) {
  const o = $e(s), c = e || pe(s, n), r = s === z.RATIONAL || s === z.SRATIONAL;
  if (r)
    for (let l = 0; l < n; l += 2)
...
class w3 {
  ...
  constructor(a, t, s, n, i) {
    this.source = a, this.arrayOffset = t, this.littleEndian = s, this.fieldType = n, this.length = i, this.data = pe(n, i), this.itemSize = $e(n), this.maskBitmap = new Uint8Array(Math.ceil(i / 8)), this.fetchIndexPromises = /* @__PURE__ */ new Map(), this.fullFetchPromise = null;
  }
  /**
  ...
  async loadAll() {
    return this.fullFetchPromise || (this.fullFetchPromise = this.source.fetch([{
      ...
      const t = new ya(a[0], this.arrayOffset, !0, !1), s = je(this.data, _e(t, this.fieldType), t, this.fieldType, this.length, this.arrayOffset, !0);
    ...
    if ((this.maskBitmap[t] & s) === 0) {
      if (!this.fetchIndexPromises.has(a)) {
        const i = this.source.fetch([{
          ...
          const o = new ya(d[0], this.arrayOffset + a * this.itemSize, !0, !1), r = _e(o, this.fieldType).call(o, n);
...
class z3 {
  ...
  toObject() {
    ...
    for (const [t, s] of this.actualizedFields.entries()) {
      const n = typeof t == "number" ? Ke[t] : void 0, i = n ? n.name : `Tag${t}`;
      a[i] = s;
    }
    return a;
  }
...
class p3 {
  ...
  async getSlice(a, t) {
    ...
    return new ya((await this.source.fetch([
      {
        offset: a,
  ...
  async parseFileDirectoryAt(a) {
    ...
    for (let h = 0; h < i; l += t, ++h) {
      ...
      const C = $e(m), k = l + (this.bigTiff ? 12 : 8), y = Ke[u]?.isArray, B = Ke[u]?.eager || this.eager;
      if (C * b <= (this.bigTiff ? 8 : 4))
        w = je(pe(m, b), _e(n, m), n, m, b, k, y);
      else {
        const S = n.readOffset(k), x = $e(m) * b;
        if (n.covers(S, x))
          w = je(pe(m, b), _e(n, m), n, m, b, S, y);
        else if (B) {
          const E = await this.getSlice(S, x);
          w = je(pe(m, b), _e(E, m), E, m, b, S, y);
        } else y ? v = new w3(this.source, S, this.littleEndian, m, b) : p = async () => {
          const E = await this.getSlice(S, x);
          return je(pe(m, b), _e(E, m), E, m, b, S, y);
        };
      }
      w !== null ? o.set(u, w) : p !== null ? c.set(u, p) : v !== null && r.set(u, v);
    }
    const f = n.readOffset(a + s + t * i);
    return new z3(o, c, r, f);
  }
}
function Ia(e, a, t, s) {
  let n = null, i = null;
...
class ft extends _3 {
   ...
   */
  constructor(a, t, s, n, i = {}) {
    super(), this.source = a, this.parser = new p3(a, t, s, !1), this.littleEndian = t, this.bigTiff = s, this.firstIFDOffset = n, this.cache = i.cache || !1, this.ifdRequests = [], this.ghostValues = null;
  }
  /**
   * @param {number} offset
   * @param {number} [size]
  ...
  async requestIFD(a) {
    ...
    if (!this.ifdRequests[a - 1])
      try {
        ...
        throw t instanceof da ? new da(a) : t;
      }
   ...
   */
  async getImage(a = 0) {
    return new Xo(await this.requestIFD(a), this.littleEndian, this.cache, this.source);
  }
  /**
  ...
  async getGhostValues() {
    ...
    if (t === Ia(n, z.ASCII, t.length, a)) {
      const d = Ia(n, z.ASCII, s, a).split(`
`)[0], o = Number(d.split("=")[1].split(" ")[0]) + d.length;
      o > s && (n = await this.getSlice(a, o));
      const c = Ia(n, z.ASCII, o, a), r = {};
      c.split(`
`).filter((l) => l.length > 0).map((l) => l.split("=")).forEach(([l, f]) => {
  ...
  static async fromSource(a, t, s) {
    const n = (await a.fetch([{ offset: 0, length: 1024 }], s))[0], i = new Yo(n), d = i.getUint16(0, !1);
    let o;
    if (d === 18761)
      o = !0;
    else if (d === 19789)
...
async function v3(e, a = {}, t) {
  return ft.fromSource(b3(e, a), void 0, t);
}
async function g3(e, a, t, s) {
  const n = a * K, i = ga - t * K, d = K + 2, o = new Float64Array(d * d).fill(NaN), c = q0(n - 1, i + 1, K + 2), r = c.map((h) => h[0]), l = c.map((h) => h[1]), f = e.filter((h) => h.bounds[0] < Math.max(...r) && h.bounds[2] > Math.min(...r) && h.bounds[1] < Math.max(...l) && h.bounds[3] > Math.min(...l));
  for (const h of f) {
    s?.throwIfAborted();
    const m = await (await v3(h.url, { allowFullFile: !1, cacheSize: 16, blockSize: 65536 }, s)).getImage(), [b, w] = m.getOrigin(), [p, v] = m.getResolution();
...
async function Ot(e, a, t) {
  if (e)
    return new Promise((s) => {
      try {
        const n = e.transaction("tiles", t ? "readwrite" : "readonly"), i = n.objectStore("tiles"), d = t ? i.put(t, a) : i.get(a);
        d.onsuccess = () => s(d.result), d.onerror = () => s(), n.onabort = () => s();
      } catch {
        s();
      }
    });
